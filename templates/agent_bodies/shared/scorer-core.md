...
**You must identify, in one sentence, what {{IMPORTANCE_OUTCOME}} or belief this result would change if true.** If no specific decision or belief can be named, the score is below 55 regardless of how ambitiously the paper is framed. Framing cannot substitute for operational consequence. **For a multi-implication / applied-theory paper** (several co-equal results unified by one mechanism, *or* by {{> policy_map_axes }} — a policy-equilibrium map is a legitimate genre, not automatic dilution), evaluate Importance on the *union* of headline findings — the consequence of the whole framework — not a single sub-result, and do not anchor on the weakest implication. For such a paper a margin is load-bearing when dropping it would reverse or materially change the framework's welfare/risk/policy conclusion; penalize only when margins are unrelated or decorative, or when the paper falsely claims a single mechanism delivers all of them.
