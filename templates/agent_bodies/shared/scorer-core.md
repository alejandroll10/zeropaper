...
**You must identify, in one sentence, what {{IMPORTANCE_OUTCOME}} or belief this result would change if true.** If no specific decision or belief can be named, the score is below 55 regardless of how ambitiously the paper is framed. Framing cannot substitute for operational consequence. **For a multi-implication / applied-theory paper** (several co-equal results unified by one mechanism, *or* by one policy instrument, welfare question, equilibrium environment, or risk decomposition — a policy-equilibrium map is a legitimate genre, not automatic dilution), evaluate Importance on the *union* of headline findings — the consequence of the whole framework — not a single sub-result, and do not anchor on the weakest implication. For such a paper a margin is load-bearing when dropping it would reverse or materially change the framework's welfare/risk/policy conclusion; penalize only when margins are unrelated or decorative, or when the paper falsely claims a single mechanism delivers all of them.
...
- **60**: the core model itself carries multiple non-load-bearing assumptions, frictions, or alternate formulations — the model that delivers the main result is doing more than the result requires. (A clean core model with many robustness checks or extension appendices stays at 80–100, not here.)
...
**An assumption added to the core model to address an audit concern or referee objection, but not load-bearing for the main result, counts against parsimony** — core bloat is the target. **It is the bloating of the *core model* that costs, not the act of addressing the concern.** The *same* concern addressed *outside* the core model — via a robustness check, an alternative specification, a separate extension, or an appendix — is not a violation, even though it adds material, because it sits outside the core model and does not complicate the main result. The test is always: does this element sit *in the core model from which the main result is derived* and fail to be load-bearing for it? If it lives outside the core — relegated to an appendix, a robustness section, or a structurally separate extension that does not enter the main derivation — it does not count. **Multi-piece exception:** when the paper's contribution is structurally multi-piece and each piece is load-bearing for the union thesis (apply the same standard as H1 — is the union statable as a single thesis only with this piece present?), the multi-piece structure itself is not a Parsimony violation — the test is whether the pieces are load-bearing, not whether they could be flattened to a single proposition. The pieces may carry distinct proximate mechanisms unified by one policy instrument, welfare question, equilibrium environment, or risk decomposition; the "40: kitchen-sink" anchor above applies only when such margins are *not* each load-bearing for the union conclusion — not merely because more than one mechanism appears.
