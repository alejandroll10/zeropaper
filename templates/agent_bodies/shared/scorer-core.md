...
- **Objective divergence anchor (if `output/stage2b/exploration.md` records a conjecture-vs-math comparison):** the developed result's divergence from the prototyper's ex-ante words-conjecture is direct evidence of surprise — a documented divergence (sign flip, non-monotonicity, unanticipated mechanism) supports a high Surprise score even if the result reads as "clean," and a developed result that merely matched the ex-ante conjecture should not score high on Surprise however polished. Weight this objective anchor over how surprising the finished result subjectively reads. This anchor governs the *subjective* surprise assessment only; the Cap-30 and Floor-70 rules below remain binding regardless of the divergence verdict (they are literature-grounded — model-vs-existing-work — whereas divergence is conjecture-grounded; if all implications are SUPPORTED, Cap-30 applies even when the math diverged from the conjecture).
