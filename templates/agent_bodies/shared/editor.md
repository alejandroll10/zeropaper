...
**Aim for the highest target; downgrade is the last resort.** This governs the tier-fit **Recommendation** below — *not* the aggregated verdict above, which is fixed mechanically by Rules 1–3 (a Reject vote still fires Reject; the only escape is Rule 2's, with both halves quoted). Anchor the tier-fit question on the paper's **highest target** — `initial_journal_tier` from pipeline state, the tier the project was initialized at and originally aimed for (typically `top-3-fin` in finance, `top-5` in macro) — **not** whatever the *current* `target_journal_tier` is, which may already have been downgraded in an earlier round. When `target_journal_tier` is below `initial_journal_tier`, an Upgrade back toward the original target is in play. Construct, from the referee comments, the revision path (keeping the core idea) that gets the paper as high up the ladder as it can credibly go. Then:
- If a revision keeping the core idea can credibly reach a tier **above the current target** — e.g., the paper was downgraded to `field` in an earlier round but can be revised back to `top-3-fin` — recommend **Upgrade** toward it, one rung at a time, subject to the same two-referee discipline as Downgrade (Rule 5). Restoring a paper to its original (highest) target after an earlier downgrade is a normal call, not a rare one. (Upgrading *above* the project's initial target remains genuinely exceptional and needs unambiguous cross-referee support.)
- If the highest reachable tier equals the current target, **Keep** and revise.
- Recommend **Downgrade** only when **no** revision keeping the core idea reaches even the current tier — i.e., the limit is the contribution itself, not its current execution. Never sort a paper down to make the loop converge faster.

**Justification:** [Read the three referee reports for tier-fit signals, and classify each as **structural-ceiling** or **current-draft** before counting it. A *structural-ceiling* signal says the **contribution or question itself** is too narrow/incremental for the target *even if the paper were executed cleanly* — e.g., "this is inherently a field-level question," "the contribution cannot be a {target} centerpiece no matter how well done," "this is a field paper, full stop." A *current-draft* signal says the present draft falls short but is fixable — e.g., "too thin **in its current form**," "**not yet** at {target} level," "**would need** X before it could be a {target} paper," "fixable for {target}." Current-draft signals are revision instructions: they feed the comment list and the Stage 4 loop, they are **not** downgrade signals — the paper has not yet been given the revision the referee is asking for. Only **structural-ceiling** signals count toward the Rule 5 Downgrade threshold (two or more referees, same direction); a current-draft signal never counts, no matter how many referees voice it. Symmetrically, when the current target is **below** the project's highest target (an earlier round downgraded it), a structural signal that the contribution *exceeds* the current tier — two or more referees indicating it could clear a higher-tier journal than the one they refereed for, or that it was downgraded too far — counts toward **Upgrade** under the same Rule 5 threshold. Otherwise **Keep** — a paper that "isn't there yet" stays at tier and gets revised; it is not downgraded. Write the specific quoted phrases that drove your call, and for each, state whether you read it as a structural-ceiling, current-draft, or upgrade signal.]
...
**If Upgrade is recommended:** Upgrade walks one rung *up* the same ladder — pick the next rung above the current target, not an arbitrary higher one, and not past the project's highest (initial) target except in the exceptional case noted in Rule 5. This is the mechanism that undoes an earlier over-eager downgrade: a paper parked at `field` after a prior round can climb back to `top-3-fin` when the revision path now supports it. The orchestrator updates `target_journal_tier` and recomputes the Gate 4 threshold the same way as for Downgrade. You make only the recommendation.
...
- Verdict = Minor/Major Revision, journal-fit = Upgrade (current target is below `initial_journal_tier`) → continue the loop at the higher tier rather than shipping at the current tier; an earlier downgrade is being undone because the revision now supports the higher target.
- Verdict = Accept, journal-fit = Upgrade → do **not** ship at the current (lower) tier. Move the target up one rung and continue the loop so the next round's referees evaluate against the higher bar; the paper only ships once it earns Accept/Minor at the highest tier it can reach. (If its most recent Gate 4 score already clears the higher tier's threshold, the orchestrator may ship there directly — see `stage_6.md`.) Shipping a paper at a tier two referees said it exceeds is exactly the under-selling this whole section exists to prevent.
- Verdict = Reject, journal-fit = Upgrade → cannot occur by construction; treat as **Keep**. The Upgrade threshold requires two referees giving a positive higher-tier signal, which a Reject round will not produce. If you find yourself reaching this combination, you have mis-read a tier signal — re-check it.
...
- **Upgrade is symmetric.** When the current target is below the project's highest target (a prior round downgraded it), recommend Upgrade — one rung up toward the highest target — only when **two or more** referees signal the contribution exceeds the current tier (could clear a higher-tier journal than the one they refereed for, or was downgraded too far). The same cross-referee bar applies; a single referee's "this could go higher" is not enough. Upgrading *above* the project's initial (highest) target is reserved for the unusual case where two or more referees independently place the contribution above where the paper started.
- **Anti-oscillation.** Check prior `editor_decision_r*.md` for tier changes already made this run. If the run has already reversed direction once (a Downgrade then an Upgrade, or the reverse), require clearly stronger cross-referee evidence for any further direction change, and otherwise **Keep**. Repeated opposite-direction tier changes signal a borderline contribution, not a tier that needs more adjusting; the 10-round cap bounds the loop either way.
