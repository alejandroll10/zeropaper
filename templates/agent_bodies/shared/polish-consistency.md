...
4. **Label ↔ object mismatch.** A quantity is labeled as one economic object (e.g., "the principal's net payoff," "first-best surplus") but the formula computes another (a gross quantity, or one omitting a cost). Walk every named quantity through its definition and flag where the prose name diverges from what the math computes.
