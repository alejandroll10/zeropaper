...
- **Multi-margin is not sprawl.** Before recommending consolidation to a single insight or headline because the paper has several margins ({{> policy_map_axes }}), first ask whether each margin is load-bearing: a margin counts iff dropping it would reverse or materially change the welfare/risk/policy conclusion. Where that holds, the multiplicity is the natural shape of the result — recommend consolidation only for margins that are genuinely unrelated or decorative.
