...
4. The domain log (`output/stage0/domain_log.md`) — one line per scan this run, `{domain} — fresh scan` or `{domain} — corrected re-scan: {correction}`; this is the record of which domains are spent and which have used their one corrected re-scan. Absent only in a run whose Stage 0 began before v2.18.1; then you have no spent-domain record and should say so in the report rather than assume nothing was tried
...
- **RESCAN-CORRECTED:** same procedure — orchestrator **also increments `problem_attempt`** — but re-scans the *same* domain with your named correction passed to `literature-scout` as an explicit instruction. Commit `pipeline: stage 0 re-entry — discovery exhausted, corrected broad scan of {domain}`. Both re-scan verdicts increment `problem_attempt` so each firing writes a distinct `branch_manager_discovery_p{N}.md` rather than overwriting the prior one. (Which domains are spent is `domain_log.md`'s job, not the reports'; the reports hold the reasoning behind each verdict.)
...
- **Recommend exactly one path.** The orchestrator needs an answer it can act on. Do not present a ranked menu.
- **RESCAN-CORRECTED fires at most once per domain.** If `domain_log.md` already shows a corrected re-scan on the current domain and discovery still exhausted, the domain is spent — do not recommend a second correction for it; recommend RESCAN-NEW-DOMAIN, or OPERATOR-ESCALATE if you cannot name a domain worth trying. This is what bounds the loop: two scans per domain, and RESCAN-NEW-DOMAIN must name a domain the log does not already hold. A second correction on the same domain is also the empirically weak move — one corrected scan that still yields nothing is evidence about the domain, not about the instruction.
- **OPERATOR-ESCALATE requires that you cannot name an untried domain.** Escalating while one remains abandons a project that has not actually run out of places to look. Conversely, do not recommend a re-scan you cannot name a domain for — "try somewhere else" is not actionable. Because the domain scope is open rather than a closed list, the stopping point is your judgment that nothing materially different is left, not an enumeration running out; make that judgment explicitly rather than escalating by default.
