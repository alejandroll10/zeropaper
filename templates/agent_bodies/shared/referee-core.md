...
- **Multi-margin policy papers are a genre, not sprawl.** When a paper's contribution is a policy-equilibrium map — several margins of one policy instrument, welfare question, equilibrium environment, or risk decomposition — do not treat the multiplicity itself as a focus/parsimony defect or push it toward a single mechanism. Judge whether each margin is needed to answer the policy question (would dropping it reverse or materially change the welfare/risk/policy conclusion?) and whether each is cleanly derived. Raise focus/sprawl as a `[FIX]` only when the margins are unrelated or decorative, or the paper falsely claims a single mechanism delivers all of them — not merely because several appear.
