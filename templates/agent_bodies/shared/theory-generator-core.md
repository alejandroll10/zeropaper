...
- The Gate 1b novelty check result on the selected idea (NOVEL/INCREMENTAL/KNOWN verdict + closest existing papers). If INCREMENTAL, the overlap the novelty-checker identified is the **constraint** to clear, not the objective — clear it by delivering a more *interesting* result (see the Gate-3 novelty reports below), not by piling on non-overlapping structure.
