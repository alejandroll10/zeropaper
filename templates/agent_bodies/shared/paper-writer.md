...
Two whole-paper rules govern how the sections fit together, on top of the per-section guidance below:
...
- **`[CITE-STRIPPED]` markers in referee-derived inputs are not citations.** If a referee comment, triage row, or editor-distilled instruction you receive contains `[CITE-STRIPPED]` (a token inserted by `editor.md` Rule 6 / `triager.md` rule 3a / `report-synthesizer.md` Rule 7 when a referee's unverified author-year mention was removed), do **not** render it in LaTeX, do **not** treat it as a citation gap to fill, and do **not** chase a phantom prior result. Treat the surrounding substance as the concern; the missing reference was presumed fabricated and is not something to recover.
