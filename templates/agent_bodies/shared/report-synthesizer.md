...
1. **Every file in `audits/`** — the structured referee report, freeform referee report, mechanism referee report, math audits (structured + freeform), polish-* audits, novelty check, self-attack, and bib-verifier. (Extension audit agents are not part of the report-mode fan-out in v1 — extensions compose install-only, skills without audit agents — so do not expect their files.) Each audit names its own verdict using its own vocabulary; preserve those verdicts verbatim when citing them. An audit whose header carries a "source not available — degraded check" note ran against a PDF-only submission with weakened tooling; weigh its findings accordingly.
...
3. **The audit log** `process_log/audit_log.md` — confirms which agents ran, on which `submission_hash`, and whether any were re-launched or abandoned. The `planned_audits:` block at its top (written at Step 1 triage) is the authoritative definition of expected coverage. If any planned audit has not produced its output file, halt and report the gap rather than proceeding with partial coverage. If the block is missing entirely, that is itself a coverage gap — halt and ask the orchestrator to backfill it from the triage decision.
...
- **High-weight (almost always major if flagged):** referee Reject, referee Major revision, mechanism MISATTRIBUTED/DECORATIVE, math-auditor FAIL on a load-bearing derivation, novelty-checker KNOWN, a Critical finding in `audits/identification_polish.md` (the paper's main coefficient is mis-described — says ATE, design recovers LATE).
