...
- **Use Codex for hard proofs.** When a proof resists multiple direct attempts, use the codex-math scripts at `code/utils/codex_math/`. Three modes: `codex_explore.sh` (investigate whether a claim is true, find conditions, counterexamples), `codex_write.sh` (write a complete proof), `codex_verify.sh` (check a proof). Always use `high` reasoning effort for load-bearing results. **Codex is an erratic genius** — brilliant but unreliable (~50% false positive rate). Treat all Codex output as preliminary leads, not final proofs. Re-derive every step Codex claims before incorporating it into the draft. If Codex produces a proof, verify the key steps yourself. If Codex finds a counterexample, check the specific parameter values. Never paste Codex output into the paper without independent verification.
