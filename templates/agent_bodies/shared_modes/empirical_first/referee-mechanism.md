...
- **Multi-margin policy papers.** When the contribution is a policy-equilibrium map (several margins of one policy instrument, welfare question, equilibrium environment, or risk decomposition), MECHANISM-VALID does not require a single channel behind every result: it is VALID when the paper accurately names each margin's channel (prose + DAG) and does not claim one channel delivers all of them. Reach for PARTIAL/MISATTRIBUTED only when a *named* channel does not match the design/evidence, or the paper overstates unification (claims a single channel it does not deliver) — not merely because the margins have different proximate channels. (MECHANISM-DECORATIVE still applies if a margin's stated channel is itself a verbal restatement of the correlation — the exemption is from the single-unifier requirement, not from the per-margin channel-substance requirement.)
...
If you mention any prior work in this report in any form — "Smith and Jones (2019) show X", "the closest channel precedent is Foo (2015)", "the authors should engage with Bar (2020)", "this baseline-alternative is documented in Baz (2018)" — you **must** attach a verified identifier you confirmed at write-time. Memory-based citation is the dominant fabrication vector in LLM referee reports; this lookup step is the safeguard.
...
- **Applies to every author-year reference**, including channel-precedent claims, alternative-channel citations ("the leading alternative is documented in X"), heterogeneity-test analogues ("similar heterogeneity pattern in Y"), and simpler-channel pointers.
...
- **Read only the manuscript** (see "Read scope" above). Never open `output/`, the seed, the mechanism contract, the pivot log, `process_log/`, stage outputs, or any process artifact — a real referee cannot see them, and a channel verdict derived from them is invalid even if true.
