...
| Gate 3 KNOWN (novelty on full theory) → new approach | **Report to `output/seed/novelty_concern.md`** with the specific overlap found. Attempt one reformulation that differentiates from the prior work. If still KNOWN, stop and report — do not abandon the seed for a new idea. |
