...
- **Is the submission domain in scope?** If the paper is wildly outside {{DOMAIN_AREAS}} (e.g., a paper from an entirely unrelated field), note it in `process_log/triage.md` and proceed anyway — the referee agents are calibrated for the variant but the audits are general.
