...
Problem selection sets the ceiling on the whole pipeline, so a round that kills its NOVEL ideas on *difficulty* and advances only an INCREMENTAL/OBVIOUS survivor is among the most costly silent failures Stage 1 can produce. This guard forces one harder idea-generation round before that happens. It fires **at most once per problem** and never costs a shippable idea (the current survivor is preserved as a fallback).
...
1. Among the S TRACTABLE survivors, rank by these criteria in order (advance to the next criterion only to break a tie):
   ...
   **Portfolio-guard fallback inclusion:** if `fallback_idea_sketch_name` is non-null (a fallback was snapshotted by Step 2a for the current problem — the pointer-lifecycle rules guarantee it cannot belong to a prior problem), include it as an additional ranked candidate here using its recorded `novelty`/`surprise` verdicts (its files are `output/stage1/fallback_idea.md` and siblings). After this tiebreak resolves — whether the fallback wins or loses — clear the pointer (`fallback_idea_sketch_name = null`) so it is not reconsidered in a later Round. If the fallback wins, treat its files as the winner in step 4 below (copy `fallback_idea.md` → `selected_idea.md`, etc.).
