...
| Case | Action |
|------|--------|
| M = 0 (all KNOWN) | No viable ideas from this top-K. Start a new Round of Stage 1 (counts toward the idea-round cap (`loops.idea.cap`)). **If starting it would exhaust the idea-round cap (`loops.idea.cap`) and `fallback_idea_sketch_name` is non-null, apply the Step 2a fallback rescue (ship the snapshotted fallback) instead of leaving for Stage 0.** |
...
| Case | Action |
|------|--------|
| S = 0 (no TRACTABLE survivor) | Start a new Round of Stage 1 (counts toward the idea-round cap (`loops.idea.cap`)). Any `BLOCKED-IMPOSSIBLE` negative results are already propagated and will constrain the next round. **If starting it would exhaust the idea-round cap (`loops.idea.cap`) and `fallback_idea_sketch_name` is non-null, apply the Step 2a fallback rescue (ship the snapshotted fallback) instead of leaving for Stage 0.** |
...
- **(d)** `loops.idea.round < loops.idea.cap` (room under the round cap).
...
3. Start a new Round of Stage 1 (increment `loops.idea.round`; counts toward the idea-round cap (`loops.idea.cap`)). Instruct `idea-generator` to target **harder, higher-novelty mechanisms** in the territory of the blocked novel ideas: pass the blocked `NOVEL` sketches and their named-alternative notes ("Most promising alternative technique"/"angle") as seeds, and tell it to push past the textbook framing rather than retreat to safer mechanisms. The prototyper retry budget (item 3 above) applies in this round too.
