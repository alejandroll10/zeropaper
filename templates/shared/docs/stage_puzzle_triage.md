...
1. **Update state.** Increment `loops.pivot.round`. **Also increment `theory_attempt`, reset `theory_version` to 1, reset `stage2b_theory_version` to `null`, reset `stage2_mechanism_version` to `null` (under `--mode empirical-first`), reset `stage2_design_version` to `null` (under `--mode measurement-first`), and reset `triaged_lit_implications` to `[]`.** The pivoted theory is a new theory version, so every theory- and empirical-scoped loop (`fix_empirics`, `data_integrity`, `method_check`, `headline_replication`, the claim loops) starts at 0 per the generic Audit-loop scoping rule — this is the same reset the canonical Regeneration entry procedure performs, and no per-counter list is maintained here. The explicitly reset fields above are non-loop version pointers / arrays that the generic rule does not cover: all downstream `_vN.md` files (math/mechanism audit, novelty check, self-attack, scorer) must use the new version numbers so they don't collide with the pre-pivot theory's files. The `stage2b_theory_version` reset prevents the Gate 4 staleness gate (`stage2b_theory_version < theory_version`) from passing on a stale pre-pivot value; step 5 below repopulates it. **The `stage2_mechanism_version` reset is mandatory for the same reason under empirical-first, and the `stage2_design_version` reset likewise under measurement-first** — `theory_version` resets to 1, so a stale pointer equal to 1 (plausible when the pre-pivot spec passed its gate first try) would false-positive Gate 4 and silently skip the design gate on the pivoted construct spec; nulling it forces the design gate to re-fire and re-set the field. **For `stage2_mechanism_version` specifically:** `theory_version` resets to 1, so a stale `stage2_mechanism_version = 1` from the pre-pivot mechanism would false-positive both the Gate 4 block AND the Stage-3a step-0 safety net, silently skipping the mechanism-plausibility gate on the pivoted mechanism; nulling it forces Gate 2 (mechanism plausibility) to re-fire and re-set the field at the new `theory_version`. Append to `pivot_history`. Set `pivot_resolved: null` (will be set true/false after the pivoted theory's empirical run).
