...
3a. **Load-bearing premise check.** Open `output/stage4/self_attack_vN.md` and verify three properties: (i) it contains at least one `**Load-bearing premise:**` (or `**Load-bearing premise [piece K — label]:**` for multi-piece contributions) line; (ii) each named premise actually appears in or is directly implied by the theory draft's stated assumptions, data choices, or methodology — not a strawman like "agents have preferences" invented for the report; (iii) each named premise is targeted by at least one Assumption-attack group. Property (ii) is an LLM-judgment check (premise membership is prose-anchored, not a string match) — orchestrator **must** read the theory draft's assumption section before deciding the premise is theory-anchored, and apply the carve-out generously when the premise is *implied* by the methodology rather than verbatim stated. **Malformed-draft fallback:** if the theory draft is missing, empty, or has no identifiable assumption / data / methodology section, default property (ii) to PASS — premise check degrades to property (i) + (iii) only, and log the degraded check in the same `process_log/self_attack_premise_skip_vN.md` file if any further property fails. Do not stall on a malformed upstream artifact. If any property fails, re-launch self-attacker once with the following branched instruction (decide single-piece vs multi-piece using the scorer-core H1 standard — multi-piece only if the contribution is genuinely two-or-more independently-load-bearing pieces stated as a union thesis; default to single-piece): "your prior report omitted or strawmanned the load-bearing premise step. **First decide:** is this paper single-piece (default — one headline result with one load-bearing chain) or genuinely multi-piece (matches scorer-core's H1 multi-piece exception — each piece independently load-bearing for a union thesis)? **If single-piece:** state exactly one `**Load-bearing premise:** …` line at the top of the output, drawn from the theory draft's stated assumptions or methodology (not invented), naming the assumption / data choice / methodological step whose removal makes the headline unprovable. **If and only if multi-piece per H1:** list one `**Load-bearing premise [piece K — short label]:** …` line per piece, each anchored and headline-critical per the same standard. Do not invent multi-piece structure to satisfy this instruction. Then attack each named premise exhaustively under Assumption attacks before any other category." Overwrite `output/stage4/self_attack_vN.md` and commit `artifact: self-attack v{N} (premise re-fire)` so the registered artifact matches what downstream agents read. If the second attempt still fails, write a one-line note to `process_log/self_attack_premise_skip_vN.md` and proceed — the scorer's advisory check and branch-manager will treat high-severity robustness attacks with skepticism, but do not loop more than one self-attacker re-fire per version N. This check is a self-attacker-side gate, not a theory-generator gate: a missing or strawman premise line never routes back to theory-generator.
