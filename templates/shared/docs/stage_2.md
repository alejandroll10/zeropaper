...
4. If FAIL:
   ...
   - Progress means the auditor's **error classes are being retired**, not that the error count is falling. A class flagged in the previous version that is still flagged after a fix aimed at it is not being patched successfully — **retire it**: either prove the general version, or cut the claims it attaches to back to what was actually verified, applying that cut to *every* claim of the same shape in the draft, not only the instance the auditor named. Retirement is verified **reactively, by the next audit** — if the following version's audit re-finds the class, it was not retired, whatever the mutate step claimed. A class that survives retirement escalates to theory failure: **increment `theory_attempt` AND reset `theory_version` to 1** (the next draft is `theory_draft_v1.md` under the new attempt). **Hard cap: 3 consecutive math-audit failures on the same theory.** At the cap, patching the current draft again is not an option — escalate, either by incrementing `theory_attempt` as above or by swapping sketches per the authority two bullets down. The cap binds on its own, whether or not any recurrence was ever judged: the class trigger can only fire *earlier* than it, never defer it. The prior-attempt audits persist across the version-counter reset and theory-generator self-reads them (its body covers the cross-attempt case), so recurring error classes from a failed attempt still inform the fresh v1.
...
4. If FAIL:
   ...
   - Same rule as Step 1: retire recurring concern classes rather than counting them down; escalate only if a class survives being retired
...
4. Route:
   ...
   - **REVISE:** re-launch `theory-generator` in **mutate** mode with `mechanism_audit_vN.md` attached, increment `theory_version`, and re-run this gate on the new version. Same progress rule as the theory-first Gate 2 FAIL path: retire recurring failed-dimension classes rather than counting them down — cut the claims a recurring class attaches to back to what the design actually supports, and where the mechanism document makes the same move at more than one site, cut every site rather than only the flagged one. **Hard cap: 3 consecutive REVISEs on the same mechanism.** At the cap, re-mutating the current mechanism again is not an option — escalate, either by incrementing `theory_attempt` (reset `theory_version` to 1) or by swapping sketches per the authority below. The cap binds on its own, whether or not any recurrence was ever judged; a surviving class fires *earlier* than the cap and never defers it. Note a dimension label is a coarse proxy for a class — the same dimension failing twice for unrelated reasons is not recurrence, and a defect that reappears under a *different* dimension label after a cosmetic dodge is. The escalation machinery from the theory-first Gate 2 FAIL path applies here with the same triggers: the **branch-manager every-3rd-version trigger** (`theory_version % 3 == 0`, launched with the current draft + audit + idea sketches + literature map) and the **pre-Stage-5 sketch-swap authority** (after 3 consecutive REVISEs on the same mechanism OR any branch-manager RESTRUCTURE verdict, evaluate swapping to a different Round-1 sketch on equal footing with continuing — record the evaluation in the commit message: name the candidate sketch(es), why continuing might still work, and the decision; continuation must be justified by specific evidence the alternative is worse, not sunk cost).
