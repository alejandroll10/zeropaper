...
<!-- THEORY_FIRST_END -->
<!-- EMPIRICAL_FIRST_START -->
Re-run Stage 3a (empirical re-fire on the extension's new prediction) + Gate 4 on extensions. Gate 2 is skipped under empirical-first; Gate 3 (novelty) re-fires only if the extension introduces a structurally new channel. **Reset `data_integrity_round` to 0** before re-entry — a deepening extension is a fresh analysis cycle (typically with new variables or new sample slices), and a stale counter from a prior pass would force-FAIL the first legitimate REVISE from the data auditors.
<!-- EMPIRICAL_FIRST_END -->
