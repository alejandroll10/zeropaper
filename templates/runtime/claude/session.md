...
Only a session that is about to *do* pipeline work runs this — it is reached from the `not_started` and `running` branches above and from nowhere else. A `complete` or `halted_*` session skips it entirely: it reports and stops, so it needs neither live data connections nor a stall guard, and on `halted_wrds_unreachable` in particular, bringing the service back up is precisely the operator-driven repair that branch forbids.
