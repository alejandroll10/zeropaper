...
- **Model-tier failure.** A model-tier outage (credits required / model unavailable / outage) is infrastructure, not a substantive failure — do not route it through the stage logic, and do not poll-wait for the tier to recover.
  ...
  - Neither runtime self-heals a tier outage (**automated** fallback-chain remap is Claude-only): codex (`gpt-5.6-{sol,terra,luna}`) has lower tiers but no remap wired to them, and grok collapses every tier to a single `grok-4.5` model in v1 so it has no lower tier to fall back to at all. Either way a tier outage is an open limitation to surface, not something the pipeline heals itself. Full doctrine: `docs/model_fallback.md`.
