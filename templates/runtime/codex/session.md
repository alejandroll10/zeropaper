...
- **How you dispatch agents depends on which runtime you are.** This `AGENTS.md` is read by Codex, Grok Build, and OpenCode; follow only the branch matching your CLI.
  ...
  - **If you are OpenCode:** the agents are in `.opencode/agents/` (each a `.md`). Spawn each with the native `task` tool, setting `subagent_type` to the filename stem (for example `literature-scout`) and providing a self-contained `description` and `prompt`. Use foreground task calls and wait for their results; do not request experimental background execution. Multiple independent agents may be dispatched as parallel task calls in one turn. Every task prompt must name all input and output paths explicitly, especially for evaluators. All agents are pinned to `opencode/deepseek-v4-flash`; there is currently no cheaper or stronger OpenCode tier in this deployment.
