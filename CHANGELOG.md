...
Data-first specs gain a machine-readable `**Live services:** [...]` line, normally `[]` because acquisition is banked before the trusted run. Gate 2 checks it against the installed runner before the audit launches, and dimension 9 now answers a "bind the channel" audit demand with the line itself. After each analysis `verify`, Stage 3a requires the published run to stay within the line. **Behavior change:** a trusted run that queries WRDS must now declare `"wrds"`. The result-bundle contract every producer reads says so.

**Marker-less carries never reach acceptance (#347).** The audit triad and the coverage audit may still carry a unit's live evidence through repair rounds over an identifier-list probe. But when every auditor passes, and before activation, the orchestrator runs `spec_audit_scope.py acceptance-carries` on each report and re-audits in full any auditor that carried a unit over a source with no update marker. A value revised in place at such a source is therefore always re-checked live before a candidate is accepted. Probe cells take a fixed form (`marker: …` / `ids-sha256:…` / `none`), and data-integrity gains a `Source probe` column and carries only on an equal probe.
...
**Data-first repairs re-acquire only what they break — banked acquisition units (item 2 of the data-mode speed pass).** Every Stage 3a repair used to be a full fresh attempt that re-pulled every class, even when the defect sat in one class's parser or in construction code downstream of the raw bytes, and a run that died before publication threw away everything it had pulled. The partition machinery (#341, #342) is now the default layout: the acquisition layer splits into per-class units (the partition's subsets when one is declared), each banked at `output/stage3a/acquisition/e{E}/{unit_id}/g{G}/` with a ledger outside the receipted run (`E` = the new run-global `dataset_acquisition_epoch`, bumped by every fresh theory identity, so a new theory never inherits an old one's pulls). A fresh attempt still gets a new `K`, fresh receipts, and the full audit quartet, but consumes every unit's current generation unchanged unless an independent artifact — an integrity/selection/coverage finding, a construction-guard failure, a debugger verdict, or a plan diff on that class's section — places the defect in that unit's acquisition. The plan gains per-class `## Class:` sections and a `## Shared construction` section with per-class code, and the data-first auditors key carry-forward on those sections (new `spec_audit_scope.py sections`), so one class's repair leaves every other class's audited evidence reusable. **#347 narrowed:** selection and coverage carry-forward (all empirical modes) now require a live source-change probe — the source's update marker, or a digest of its current identifier/event-key list — to match the prior row; what remains open is in-place value revision at marker-less sources.

## 2.47.0

**Data-first drops Gate 3a-feasibility — the Stage 1 pilot answers its question.** Under data-first the gate could only FALSIFY on core replication statistics being *uncomputable* (a wrong sign was recorded and passed through), yet it cost a full quick-pull receipt cycle on sources the pilot had already pulled. The data-first `idea-prototyper` now carries a "Replication computability" section — compute the 2–3 core replication targets on its slices to show they *can* be computed, without comparing values — and the spec audit's pilot-consistency dimension requires each target's computability to be pilot-observed or given a field-and-join path. A target that proves uncomputable after the spec is written is a spec gap flagged by the Stage 3a plan and returned to Gate 2 (narrow or replace), not an architecture kill. The marker resolver gains a `NOT_DATA_FIRST` family for blocks every other mode shares verbatim; non-data-first builds are byte-identical.

## 2.46.0

**Headline gate at publication — a report without valid headline claims is refused by the runner, not repaired by a fresh attempt (#327, deep half).** The result receipt binds the analysis report's full bytes, so a report published with a missing or malformed `## Headline claims` section could only be fixed by a whole new producer attempt. That attempt was routed through the orchestrator, preceded by a wasted replicator launch, and charged to the unowned-failure counter. In autonomous deployments, `run-empirical` now parses every declared `output/stage3a/empirical_analysis*.md` report in the isolated workspace with the replication manifest's own parser (`empirical_input_manifest.py`), before anything is published. A missing section or malformed row refuses the run with nothing published and says exactly what to emit. The empiricist fixes its code and reruns the same command in the same firing. Manual deployments are not gated. A missing parser, or a missing or malformed `.deploy_manifest.json`, fails closed. The runner's in-process utility loads no longer write `__pycache__` under `code/`, because a gitignored bytecode cache there is part of the code surface headline replication binds, and it would read as a code change after a fresh checkout. Step 6.5's `no_headline_tags` branch shrinks to a replicator self-failure backstop, and its unowned-build example is gone. The #327 LIMITATIONS entry is closed.

## 2.45.0

**Scoped Gate-2 re-audit in data-first — a one-dimension spec fix re-buys the dimensions it touches, not all eleven (#345).** A mutate that fixed exactly one flagged dimension of the dataset-specification audit used to pay the full 11-dimension audit again (incumbent lookups, rights cross-checks, pilot reconciliation) on sections that had not changed, up to three times across the mutate cap. Gate 2 now passes the spec auditor its own report on the previous version plus that version's spec. A new helper, `code/utils/spec_audit_scope.py`, digests the spec per `##` section (fenced code never splits), digests the rights inventory with its per-version `dataset_version` field removed, and digests the pilot, problem-statement and build-report inputs. Every report now records that block, including the spec and rights paths it digested. `compare` recomputes the prior side from those files, fails closed unless they match the recorded digests, and prints the changed sections and inputs plus the full spec and rights diffs. The recorded digests are never the source of truth. The auditor reads the whole diff against every dimension and carries a dimension's clean prior assessment forward only when nothing in the diff bears on it. A changed input, a helper refusal, a dimension the last round failed, or any doubt means re-assessment, and any changed pilot, problem statement or build report means a full audit. Local checks re-read only changed sites; cross-section checks re-read every participant. Dimension 8's routed lines and dimension 4's `dataset_version` check always re-run. The orchestrator never asserts what changed, the audit still issues one fresh verdict over all eleven dimensions, and carried paragraphs cite their root version so byte identity chains hop by hop. The census leg is untouched. Residual limit: whether a diff bears on a dimension is the auditor's judgment, not a mechanical test (LIMITATIONS.md, #349).

## 2.44.1

**Busy is busy everywhere — WRDS contention can no longer read as a stale daemon or an outage (#346).** The query bridge answered relay-slot exhaustion with the terminal `safety` kind, and the daemon silently closed connections beyond its 32-thread cap; either could turn a burst of concurrent commands into operator-escalation guidance or a false `halted_wrds_unreachable`. Both now answer `busy` (the daemon from a small separate reply pool, and only to clients that declare `busy_aware` — released v7 clients read any non-ok handshake as a terminal latch, so they keep the old silent close), the client absorbs a busy handshake exactly like a busy command under the one shared backoff budget, and ping, bridge ping, latch diagnosis, `unblock`, and the watchdog all read busy as alive-but-contended. The same pass closes a #322 gap for sandboxed Linux clients: the bridge answered a dead, dropping, or stalled daemon behind it with the `safety` kind, which skipped the watchdog-repair wait and made `wrds_auth_error()` report a false latch; it now answers `unavailable`, which the client maps to an ordinary connection error. The daemon and relay are long-lived and host-wide, so an already-running pair keeps its old behavior until restarted (no protocol bump: that would force a restart and a Duo login on every live run); the daemon now reports its build in the handshake and `wrds_client.py status` says when it predates the deployed code.

## 2.44.0

**WRDS daemon self-heal — a dead, wedged, or orphaned daemon is repaired without an operator, and never at the cost of an unapproved Duo loop (#322).** `./launch.sh` now registers its PID with a host-side singleton watchdog (`code/utils/wrds_watchdog.py`) that, while any registered launcher still needs WRDS, restarts a daemon that died, stops (birth-token-verified TERM→KILL) and restarts one that failed every health probe for ten minutes with no in-budget command, and replaces an older-protocol daemon whose deployment has no live launcher. Each restart is one ordinary `start_services.sh` start — one login, one Duo push — bounded by the existing durable latch: a login that fails, an unanswered push included, latches until the operator's `unblock`, and a successful one re-arms the allowance. `WRDS_AUTO_RELOGIN=0` turns off every unattended login, including the daemon's own reconnect after a dropped connection. The watchdog never acts on a latch or a live login attempt, stops after three repairs that never reached a login, and refuses to execute any of the six WRDS scripts if its bytes changed since the operator launched. Fixing it exposed why manual restarts had needed a stop-everything: the first-v5-upgrade namespace guard ran on every start, so any live sandboxed run blocked the daemon from starting; it now ignores v5+ deployments, whose sandbox client cannot spawn a daemon. Clients wait out a live repair (`wrds_await_service()`, the `wrds_client.py await` CLI for the Stage 3a preflight) and re-send a command that lost its endpoint (read-only, bounded by `WRDS_HEAL_WAIT_SECONDS`); the Codex driver resumes a `halted_wrds_unreachable` run once the daemon answers (bounded by `WRDS_HALT_RESUME_WAIT_SECONDS`), a Claude session lifts that halt on a passing ping, and `wrds_client.py unblock` stops a latched daemon itself instead of sending the operator to find its PID. Residual limits are tracked in #348.

## 2.43.0

**Gate-2 runner-capability preflight — an unexecutable spec fails at the spec gate, not at receipt publication (#307, cheap half).** Trusted run plans can declare inputs only as regular files (bound read-only, content-hashed) plus the networked analysis run's IP network capability — and that is enough for ordinary live pulls: `wrds_query()` and the FRED/EDGAR clients already work inside the analysis run on the declared network capability alone, with the runner binding the client state the transport needs. What the runner structurally cannot do is treat a non-file primitive as a declared input. Field evidence (tradingdays): a spec-audit thread asked for the WRDS relay channel itself to be bound and hashed into the trusted run, the spec complied, Gate 2 accepted it, and five consecutive attempts then died at receipt publication — one full build each — on a binding the runner correctly refuses (a socket is not a regular file). Now: the data-first Gate-2 audit's dimension 9 REVISEs any spec requiring a non-regular-file primitive (raw AF_UNIX socket, PID/lock file, credential relay) to be declared, bound, or hashed into a trusted run — while explicitly sanctioning standard-client pulls, which need no binding; the spec author's template carries the matching never-require rule, including the answer to give a future audit thread making the same demand; and Stage 3a's step-4 plan review asks the same question of the plan. The corrections named are drop-the-binding or bank-a-pre-run-pull-and-declare-the-file. The deep half — a declared-capability channel in the runner itself — remains tracked in #307. Also in this release: #301 closed (the v2.33.4 orphaned-plan fix confirmed with its test coverage; deployments older than v2.33.4 still need updating to receive it).

## 2.42.0

**Audit carry-forward — one repaired class no longer re-buys live verification of every untouched one (#344).** The post-empirics audit triad/quartet (`data-integrity-auditor`, `data-selection-auditor`, `coverage-auditor`) re-fires after every repair, and until now each firing re-ran every live leg — source re-queries, relaxed-filter enumerations, second-source sampling — for every cache, universe, and event class, including ones the repair never touched and that already passed with real network verification the round before. Now each auditor records a per-unit `## Scope digests` table (cache/table/log bytes + the construction/validation code + the governing plan or spec, all SHA-256, computed by the auditor itself), and on a re-fire at an unchanged `theory_version` the orchestrator passes the auditor its own prior report as `PRIOR_AUDIT_REPORT`: a unit whose full evidence scope the auditor re-digests as byte-identical to a zero-finding prior row may carry that row's live re-query evidence forward, marked with its root round. The firing itself and its fresh verdict are never skipped, no verdict is ever replayed, and the auditor's own recomputed digests are the sole qualification — the orchestrator never asserts what changed. Code digests match as content sets, never by filename, so the fresh-attempt transition's per-round entrypoint renames do not defeat carry; carried-from-carried chains resolve transitively through the immediate prior row (the root report is overwritten). Drift-sensitive legs that stay always-live: the coverage certificate's re-enumeration and full-key diff (never carried, when REQUIRED) and data-integrity's `cache-stale-vs-source` check; the residual intra-campaign source-drift exposure for units with no such leg is a documented limit (LIMITATIONS.md, #347). Method-checker is deliberately untouched (it reads code, not sources). This is the quartet-level completion of the stored-input-guard pattern (#327's shipped half, v2.36.0) and cuts the per-repair wall-clock and shared-daemon load that compound with #343 under two concurrent campaigns.

## 2.41.0

**WRDS daemon busy backpressure — contention is retryable, never a failure (#343).** The host-wide WRDS daemon serializes every query from every deployment behind one lock; before this release, queue wait, execution, and recovery shared one deadline, so under contention (two concurrent campaigns, or a sibling deployment on the host) pure queueing could exhaust the budget and surface as a generic `query`-kind error the producer cannot distinguish from its own bug — misrouting into `loops.unowned_failure` and, at caps, into a Stage-2 return for a spec that was never the problem. Now: (1) lock acquisition has its own budget (60s); exhaustion raises `WrdsServerBusy` strictly before any database or credential activity and reaches the client as a new retryable `error_kind: "busy"`; (2) the execution+recovery deadline starts only once the lock is held, so a queued command keeps its full requested database allowance; (3) the client absorbs busy answers with bounded jittered backoff inside the attempt (default 30 min total, `WRDS_BUSY_WAIT_SECONDS` overrides) and then raises typed `WrdsBusyTimeout`, documented in the WRDS skill and Stage 3a as an environment interruption — never an outage, producer, or spec defect; (4) only `busy` is ever retried — auth/safety stay terminal, and the busy path is provably credential-free, so the account-lock invariant is untouched (no `SAFETY_PROTOCOL` bump). Client and bridge response-header budgets grew by the queue allowance. Rollout note: the changes activate on the next daemon restart; until then new clients see the old contention behavior (safe), and after it a not-yet-updated deployment sharing the host may time out early under heavy contention instead of waiting out the queue — merely suboptimal, update the deployment to clear it. The bridge's own slot-exhaustion answer still reports `safety` (documented limit, #346).

## 2.40.0

- data-first: **construction partition required when parallelism is free** (#342). v2.39.0 wired `## Construction partition` into the plan, execution, and audit layers but never gave the spec author the section — dead code fixed: the theory-generator spec template now carries it. It is **required** whenever the core (non-staged) classes form two or more source-disjoint groups, computed from a new machine-readable `**Class sources:** [...]` line each Validation-plan class entry begins with (spec author, Gate-2 auditor, and plan reviewer all read the same arrays; a missing or inaccurate array is REVISE). Two exhaustive forms: a declared partition — subsets may bundle several classes, but every bundle must name the build-time dependency binding them, judged as skeptically as a serial justification, so the degenerate one-subset partition cannot launder serial construction past the gate — or a serial justification, valid only when the named dependency web connects every core class (a deferrable or merely-local dependency is REVISE; a merely-local one belongs inside a multi-class subset). A shared source never justifies a bundle: source-sharing subsets already serialize at schedule time. Missing section at ≥2 groups is REVISE — free parallelism silently defaulted to serial is a spec defect. Five independent review rounds; blocking findings (missed trigger disambiguation, subset==class contradiction, degenerate-partition escape) each fixed and re-reviewed.

## 2.39.0

- data-first: **partitioned acquisition** (#341). A spec may declare `## Construction partition`: the acquisition layer — and only the acquisition layer — splits into per-subset campaigns run concurrently (one `empiricist` per subset, source-disjoint subsets only: any shared `source_id`, chosen or triangulation, serializes, so per-source pacing/session/single-shot discipline holds unweakened). Every admission-schema interface is subset-qualified (namespaces, ledgers, `rehearsal_v{N}_a{K}_s{subset_id}` records) so concurrent campaigns cannot race. A subset failing terminally retires and relaunches alone — banked subsets' caches and ledgers carry forward, `K` untouched — and no per-subset counters exist; campaign-scoped loops bound total failures. The receipted analysis run stays single and atomic: it fires only when every non-staged subset's ledger is complete, consumes the banked caches as declared inputs, and executes the deferred cross-subset obligations; receipts, audits, activation, and the paired release are exactly the unpartitioned quartet (zero `results_pipeline.py` changes — its single-pending/exclusive-lock design is respected, not fought). Spec auditor gains dimension 11 (partition honesty: independence real, obligations deferred not dropped, concurrency-only never scope). Effective parallelism = distinct sources, not classes.

## 2.38.0

- Audit loops: **two same-shape rounds bind early** (#338). A new generic loop rule in `core.md`: when a loop's last two rounds failed on identically-named findings with no measurable progress — same items, unchanged severity/count/scope, dominant remainder flat — the loop is treated as capped now and takes its FAIL route, whatever `round` reads. Repeated verdict *classes* never trigger it; `referee` and `gate4_scorer_evaluations` are exempt (their FAIL routes are already round-keyed); seeded-mode negative-result budgets are not exempt. The escalation table, seeded Gate-2 override, and data-first seeded reading now state their round counts as ceilings, not guarantees. Field evidence: eventcal spent twelve closure-proof laps against a flat ~27,880-edge remainder and needed two operator directives to stop.
- data-first spec audit: **verification obligations are apparatus** (#338). Dimension 9's proportionality test now covers spec-imposed proof/closure obligations — a whole-artifact static proof where staged/per-change closure gives the same guarantee is REVISE on disproportion alone — with an explicit carve-out for the machinery the pipeline itself mandates (dimension 8's census, the Stage 3a admission contract).
- Stage 3a data-first: **apparatus-scoped replan** (#339). When an attempt died on a defect an independent `debugger` verdict locates strictly in plan-owned apparatus (fixture harness, wrapper, validator, admission-schema constant) and the fresh plan's spec-facing sections are byte-identical to the last approved plan (verified by diff of committed copies, never by the producer's label), the step-4 review scopes to the changed apparatus sections. Counters and the fresh-attempt transition are untouched. Field evidence: tradingdays a150's fixture rig broke and the routing cost a full replan plus a from-scratch 98-item review of an unchanged plan.
- Loop-counter semantics **deduplicated** (#340). The Loop Registry rows and `core.md`'s exception narratives are the single owners of scope/reset semantics; the stage-3a restatements (audit_fix scoping, re-fire preamble rationale) and the over-long `unowned_failure`/`stage3a_upstream_return` exception paragraphs shrank to operations plus pointers. Independent review verified zero semantic loss across three rounds.
