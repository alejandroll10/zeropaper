...
| `--ext theory_llm` | `UF_API_KEY` (from [UF NaviGator](https://api.ai.it.ufl.edu)); `DEEPINFRA_TOKEN` for cross-family replication on open-weight families. Optional frontier tiers (pay-per-token): `OPENAI_API_KEY` (GPT-5.x, from [OpenAI](https://platform.openai.com/api-keys)) and `ANTHROPIC_API_KEY` (Claude, from [Anthropic](https://platform.claude.com/settings/keys)). Or run against a self-hosted model with no key at all: set `LOCAL_LLM_MODEL` (+ `LOCAL_LLM_BASE_URL`, defaults to Ollama's) |
...
| **theory_llm** | `--ext theory_llm` | Stage 3b: test predictions via LLM experiments — gpt-oss models (UF NaviGator, free) plus optional DeepInfra open-weight families, OpenAI GPT-5.x, Anthropic Claude, or a local server |
...
**OpenCode** — `./launch.sh opencode` wraps the persistent server and every attached client in Anthropic Sandbox Runtime; `--once` wraps the whole TUI. Seatbelt (macOS) / bubblewrap (Linux) confines the descendant trees to project, project-scoped OpenCode state, and approved cache/runtime writes, and denies reads and writes under `~/.ssh` and `~/.aws`. On an empirical host it can also establish the shared WRDS daemon through a separate long-lived SRT service profile: a protected stdlib gatekeeper and host-wide startup lock run before any model-writable project interpreter or code, and the host driver validates PID/birth/group identity plus host-visible PID semantics before approving startup. Project code, `.env`, and the venv are evaluated only inside that narrower service sandbox; absent WRDS credentials produce a clean skip. The SRT policy, adapter, launcher, OpenCode config, project `.env`/deployment manifest, host driver, and host control state are immutable from inside the model sandbox; the host driver also uses an isolated Python and a PATH with every sandbox-writable directory removed, and runs Git-based progress inspection inside SRT. Network egress remains unrestricted because literature, package, and data hosts cannot be known in advance. The adapter fails closed if SRT or its policy is missing or invalid. Bare `opencode` bypasses this boundary; always use the launcher. See `LIMITATIONS.md` for the external-runtime and global cache/Codex-state caveats.
