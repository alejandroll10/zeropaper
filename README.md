...
| `--light` | Drop every subagent to the cheapest tier its runtime offers (cheaper/faster), and drop each agent's pinned reasoning effort with it. Applies to all four runtimes through their own tier tables — claude `sonnet`, codex `gpt-5.6-luna`, gemini `gemini-3-flash-preview`; grok has a single model, so it is already a no-op there. The orchestrator model is unchanged. Good for drafts or iteration. |
