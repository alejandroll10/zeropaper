...
## Operator failure mode observed three times on 2026-09-05 — read this before touching a cap
...
3. **Filed #313 claiming the R4 bootstrap is rewritten from scratch each attempt**, so fixes never
   accumulate, and proposed exempting never-receipted apparatus from in-place-edit immutability.
   Falsified before shipping: between a155 and a156 all three SHA-256 constants, the 13-element
   install order, every package pin, the identity byte string, all nine argparse flags and env
   vars, and all eleven `closure_checks` keys carry across byte-identically **plus three new
   ones**, and a156 fixed two latent a155 bugs. My evidence was renamed functions and a 420→167
   line drop that is 4-space indentation becoming 1-space. Issue closed as invalid, LIMITATIONS
   entry rewritten as a withdrawal. **The real mechanism is the opposite:** each attempt is
   stricter than the last, a stricter guard is new surface, and the defects are generated by the
   guards that catch them — so seeding from the prior file would have accelerated it.
...
retroactively. **Guards for the operator, in order of how much they actually helped:**
1. **Review before shipping, not after.** All three were caught by review agents and none by me
   noticing. The first two had already reached live deployments and had to be reverted; the third
   was reviewed first and cost only a filed issue. That ordering is the only guard that worked.
2. **Check the load-bearing evidence before building the theory on it.** All three diagnoses
   rested on something checkable in under a minute — a debug directory listing, an exception list,
   a set of shared constants — and in all three I reasoned from a shallower signal instead.
3. Note the common trigger: a counter that keeps firing, and a mechanism invented to explain why
