...
7.5. **Data-integrity audit (content + selection).** `empirics-auditor` PASS verifies the code reproduces bit-identically from the cache — it does NOT verify the cache itself is correct. Launch `data-integrity-auditor` and `data-selection-auditor` **in parallel** (single message, two Agent calls) on the empirical plan, analysis, code, and cached datasets. Each re-queries the source database (WRDS / FRED / EDGAR / FFIEC / etc.) on a sampled subset of identifiers: the integrity auditor verifies cached field values against source (Class 1 — content), the selection auditor enumerates candidate identifiers under the documented inclusion rule with relaxed filters and diffs them against the cached universe (Class 2 — sample). Each returns PASS / REVISE / FAIL with severity-ranked named failure modes; outputs go to `output/stage3a/data_integrity_audit.md` and `output/stage3a/data_selection_audit.md`. Aggregate the two verdicts:
   - **Both PASS**: proceed to **puzzle-triage entry check** (next section). Reset `pipeline_state.json:data_integrity_round` to 0.
   - **Any REVISE (and no FAIL)**: re-launch `empiricist` with both reports and the named failure modes the empiricist must fix. The empiricist patches the data-construction code (not the identification design — if a finding straddles identification, route to step 3 instead). **Increment `pipeline_state.json:data_integrity_round`.** Re-run step 7 (empirics-auditor) and then step 7.5 on the rebuilt cache. **Hard cap: 3 data-revision rounds.** If `data_integrity_round >= 3` and either auditor still returns REVISE, treat as FAIL.
   - **Any FAIL**: hard escalation — the cached data is fundamentally unreliable for the claimed population or claimed semantics. Do NOT proceed to puzzle-triage or Gate 4 on a FAIL. Two sub-cases: (a) **source-unreachable FAIL** (the auditor could not re-query because WRDS/FRED/etc. is down): retry the preflight at the top of this file once, then if still unreachable set `pipeline_state.json:status = "halted_data_audit_unreachable"` and stop; (b) **substantive FAIL** (the auditor re-queried and found a load-bearing data problem): re-launch `empiricist` with the FAIL report and an explicit instruction to redesign the data layer (re-source, re-filter, re-construct the universe), then re-enter step 5 (execute). A substantive FAIL counts as a re-execution, not a revision round; do not increment `data_integrity_round` and do not let `data_integrity_round` cap the FAIL response.
   - The audit re-fires on Stage 3a re-entry (see "Re-fire on theory revision" below) and on any cache regeneration (`mtime` advance on any cached file referenced by the analysis).
8. Commit: `artifact: empirics audit — {PASS/FAIL}; data audits — {PASS/REVISE/FAIL × 2}`
...
5. Run `empirics-auditor` on the new analysis (same audit-fix loop as the first pass; same 5-attempt cap). On PASS, re-run step 7.5 (data-integrity + data-selection auditors in parallel) on the regenerated cache — re-entry resets `data_integrity_round` to 0 and applies the same 3-revision cap.
