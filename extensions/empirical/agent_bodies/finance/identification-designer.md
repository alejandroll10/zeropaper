...
6. **State who sets the outcome.** If a strategy pools multiple parties in a transaction where one outcome (price, terms, rating) is set by only one of them (e.g. lead arranger vs. syndicate participants) — name the setting party explicitly and either restrict the treated set to it or defend pooling as the intended estimand. A non-controlling unit pooled into the treated set is a treatment-attribution error (`treatment-attributed-to-non-controlling-unit` in the auditor's `estimand-mismatch` family).
