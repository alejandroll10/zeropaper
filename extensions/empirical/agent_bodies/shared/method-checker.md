...
2. **Enumerate every named econometric method used in the empiricist's code.** Scan for:
   ...
   **Multi-version post-pipeline files.** If the same method appears in multiple `empirical_post_v*.py` files, the highest-numbered file's implementation is authoritative (referee revisions accumulate; the latest version is what the paper ships). Flag the method only if the latest file containing its implementation uses a custom approach without justification. If an earlier version had a custom implementation and a later version replaced it with a canonical import, the earlier file's custom code is obsolete — do not re-flag it; do note in the report which version you treated as authoritative.
...
3. **For each enumerated method, identify the canonical package using the skill's lookup recipes plus your own knowledge.** In order of preference:
   ...
   For each method, record: (i) does a canonical exist in the empiricist's working language? (typically Python — read `code/empirical.py`'s shebang and imports to confirm); (ii) what is the package name; (iii) is the canonical author-maintained (highest authority — original method authors maintain the package) or community-maintained.
