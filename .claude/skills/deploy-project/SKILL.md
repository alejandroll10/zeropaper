...
The v6 binary framing change is intentionally incompatible with a live v5 daemon/relay. After updating a deployment, stop the old host service once and relaunch through `launch.sh`; the safety handshake then prevents mixed-version queries. V6 replaces the accidental 90 MiB ceiling with a documented 512 MiB malformed-peer wire bound plus total frame deadlines, while the WRDS skill enforces tighter row/materialization budgets and windowed pulls for genuinely large extracts.
