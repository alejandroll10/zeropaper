...
update.sh                    # Repo root. Authenticated updater launcher and build input
scripts/update_coordinator.sh # Authenticated updater coordinator and build input
scripts/                     # Other repo-root scripts are dev-only tooling
...
coordinator additionally authenticates root `setup.sh`, `update.sh`,
`scripts/update_coordinator.sh`, `VERSION` for stamping, `LICENSE` for production
bootstrap, and `.env.example` to seed deployed configuration. Root
...
- `identification-designer` — designs the identification strategy (variant-specific: finance handles Stage 1 empirical-first and theory-first Stage 3a; macro handles its supported theory-first Stage 3a, `--ext empirical`)
- `identification-auditor` — adversarial audit of the Stage 3a identification strategy (variant-specific for finance and macro, `--ext empirical`)
...
**Checker independence.** `CLAUDE.md`'s *a checker is never the thing it checks* constrains agent design, and **`category` is not the mechanism**. That field has exactly two consumers: faithful-mode contract injection for `developing` agents, and the Grok assembler's `agents_md: false` for `evaluator` ones — the second *is* genuine input-set independence, but only on that one runtime and only against ambient orchestrator context, never against the producer's own output. Several real checkers are `developing` (`bib-verifier`, every `polish-*`, `last-resort`). Independence is bought by the input set, so when adding or reshaping any agent, ask whether an artifact the pipeline routes on could be checked by its own producer in the same firing; if it could, that check needs a separate agent, or at minimum a separate instance whose input set excludes the producer's source. Three existing patterns buy that independence in different ways, and are worth copying rather than reinventing: `headline-replicator` forbids a trivially-equivalent path and routes its own trivial paths back as a self-failure; `construction_guard.py` binds to receipt fingerprints the runner writes only after the producer exits; `coverage-auditor` re-does a stratified sample of the work itself instead of reading the builder's log. A producer-authored check is a developer convenience, never evidence, and never a routing predicate.
