...
which runs for both `--local` and production and is the only writer of those paths (the
...
`deploy_assets/` is the complete build-input tree: everything `setup.sh` reads to
assemble a deployment lives under it (templates/, scripts/, extensions/, plus the
verbatim-shipped `launch.sh` and `dashboard.html`). Dev-only tooling stays at the
repo root and is never read by a build.
...
Root `setup.sh` is a fail-fast coordinator. Build behavior lives behind explicit sourced-module interfaces under `deploy_assets/scripts/setup/`; the modules are build-time-only and never enter a deployment.

1. `resolve_config.sh` parses and validates the CLI, expands legacy/implied extensions, preserves and deduplicates extension order, and resolves variant/mode descriptors. Unknown-extension rejection remains at the historical extension boundary so diagnostics and sequencing do not change.
2. The coordinator installs one shared EXIT cleanup trap, creates tier-vocab build state, then either resolves a `--local` output directory or fetches production build inputs into a throwaway source clone. Production creates an empty project directory; no clone-and-strip step exists.
3. `finalization.sh:setup_initialize_project_git` initializes only the production project's repository. `ownership.sh:setup_ownership_init` then creates the structural ownership registries. Root infrastructure copies use `infrastructure_copy_file`; `LICENSE` uses `bootstrap_copy_file`.
...
7. `provisioning.sh:setup_python_environment` creates the production-only `.venv`, installs core dependencies, and installs the dotenv guard. `skills_and_utilities.sh:setup_skills_and_utilities` assembles core skills/utilities and invokes the same provisioning module for variant-gated SSJ dependencies.
...
9. `ownership.sh:emit_deployment_manifest` derives the replacement and merge lists from the producer registries; there is no hand-maintained candidate list. The exact historical array order is retained through numeric compatibility order plus lexical tie-breaking.
10. `finalization.sh` validates local placeholder resolution and reports the output, or, in production, finalizes the dashboard, commits the initial project, performs GitHub publication only under explicit `--publish`, and prints launch instructions. The shared EXIT trap removes all build-time temporary state.

The full 39-shape characterization fixture in `test_scripts/test_setup_characterization.py` compares complete trees, file modes, empty directories, symlinks, manifests, and CLI contracts. `test_setup_ownership.sh` covers update preservation and adversarial ownership paths; `test_setup_publish.sh` exercises the committed production clone, provisioning order, git commit, and every publish-safety branch with fake `uv`/`gh`.
