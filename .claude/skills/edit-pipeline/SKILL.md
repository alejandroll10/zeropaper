...
- **Codex's bundled skill-authoring validator caps `name` at 64 characters and `description` at 1024 characters — characters, not UTF-8 bytes.** The source of truth is `codex-rs/skills/src/assets/samples/skill-creator/scripts/quick_validate.py`, which uses Python `len()` for both fields. `sync_dev_instructions.sh` and `deploy_assets/scripts/assemble_codex_skills.py` therefore fail above either cap. The current runtime loader is more permissive for an overlong description, but generated skills must satisfy the authoring contract rather than rely on that implementation detail. A separate aggregate skills-metadata budget across all rendered descriptions also truncates — and it is **tokens**, not characters: `skill_metadata_budget()` returns `SkillMetadataBudget::Tokens(2% of context window)`, falling back to `Characters` only when the context window is unknown, so openai/codex#24299's `budget_limit=5440` is `272_000 × 2%` tokens. It depends on the model and on what else is installed, so it is not checked.
...
3. Assembles runtime docs (CLAUDE.md, AGENTS.md, GEMINI.md):
   ...
   - Injects runtime-specific session guidance from `deploy_assets/templates/runtime/{runtime}/session.md`
   - Substitutes per-variant scorer calibrations from `deploy_assets/templates/agents/{variant}/vocab.json` into the scorer agent body
   - If `--seed`: injects `deploy_assets/templates/shared/seed.md` as `{{SEED_OVERRIDE}}`
   - If `--faithful`: injects `deploy_assets/templates/shared/faithful.md` instead (supersedes seed.md at the same placeholder)
...
5. Decide extension/mode compatibility explicitly: `--ext empirical` requires an `deploy_assets/extensions/empirical/agent_metadata/{variant}_agents.json`; `--mode report` requires a `deploy_assets/templates/agents/{variant}_modes/report/vocab.json` overlay *and* domain-appropriate shared audit bodies (see the llm_cognition entry in LIMITATIONS.md for why that variant gates both off in `setup.sh` instead).
