...
**Known limit — silent drift.** Nothing in git enforces that the generator ran. Edit `CLAUDE.md` under a Claude session, commit, and a later Codex session reads stale instructions with no warning. `scripts/test_sync_dev_instructions.sh` validates the **index** — what a commit would record — so stage the mirrors before running it; it never writes, and the idempotent generator is what tells you you were out of sync in the first place. Run it after touching either canonical source — **including after adding a dev skill**, which is the one case where the old "adding a skill is zero-maintenance" instinct is now wrong — and from the mandated post-change review. It is not *automatic* — closing that fully needs a pre-commit hook or CI, neither of which this repo has.
