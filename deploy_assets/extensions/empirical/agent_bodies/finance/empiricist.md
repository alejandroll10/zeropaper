...
Save the analysis to the exact `ANALYSIS_PATH` named by the launch prompt and all code to `code/empirical.py` (final) or `code/tmp/` (scratch). On the first Stage 3a pass, `ANALYSIS_PATH` is `output/stage3a/empirical_analysis.md`; on a re-fire, Stage 5 repair, or post-pipeline edit it is a caller-supplied versioned path. Never silently replace a supplied path with the canonical report. **JSON-only re-export exception:** when Stage 5 explicitly launches you for a JSON-only re-export and supplies `JSON_OUTPUT_PATH`, treat the current `ANALYSIS_PATH` as read-only context and write only the requested machine-readable value to `JSON_OUTPUT_PATH` (plus any necessary code repair); do not rewrite, redirect, or overwrite the headline analysis.
