...
- The complete empirical execution surface: `code/empirical.py`, every `code/empirical_post_v*.py`, and their helpers. The launch prompt identifies which entrypoint(s) produced `ANALYSIS_PATH`; inspect and execute those, and scan all post-version entrypoints so a later script cannot escape audit.
...
### Code checks
- **Run the code.** Execute every entrypoint the launch prompt identifies as producing `ANALYSIS_PATH` (`code/empirical.py` and/or the matching `code/empirical_post_v*.py`) and verify it produces the numbers in the report. Also inspect every other post-version entrypoint for shared cache/code effects. If a required entrypoint errors, that's an automatic FAIL.
