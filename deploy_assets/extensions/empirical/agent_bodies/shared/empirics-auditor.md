...
- Every analysis contract and audit-only execution summary resolved from `RESULT_PLAN.analyses`, plus their shared immutable baseline. Validate each with `analysis_contract.py`. Treat the effective contract—not remembered defaults or the prose plan—as the executed specification authority; use the prose plan to judge whether that authority represents the intended science.
...
- The empiricist's scratch code (if any), which you read but never write to: `code/tmp/*.py`; per-analysis headline-replicator scripts are separate artifacts under `output/stage3a/verification/`.
- The theory draft (to verify tests actually match predictions)
- The implications (to verify the right things were tested)
- **The headline-replicator's per-analysis result named by the launch prompt as `VERIFY_RESULT_PATH`** — per-claim `{reported, replicated, relative_delta, agree, path_class}` for every `[HEADLINE]`-tagged claim, plus `untagged_warnings`. Before using its claim rows, run `python3 -I -S code/utils/empirical_input_manifest.py compare --result VERIFY_RESULT_PATH --analysis ANALYSIS_PATH`, substituting the two exact launch-prompt paths. A nonzero exit, any status other than `UNCHANGED`, a non-PASS enclosing verdict, or a manifest naming another analysis is an automatic FAIL: the replication evidence is absent or stale for this audit. Otherwise read it as authoritative on whether each headline number survives an independent-path recomputation.
- **Every construction-guard report named by the launch prompt as `CONSTRUCTION_GUARD_REPORT`** (one per receipt; data-first runs supply the release receipt's report as well). The guard is mechanical and narrow: it checks that each row's claimed content digest is the digest of the declared producer input that same row names, that source references resolve to declared inputs, and that locator fields can address something per row. Where a row names no declared input the digest could only be checked for membership in the receipt's digest set, which is a weaker claim; the report says so per column in its `binding` field and reports such a column as a warning rather than a pass, and that column is yours to judge against the specification. The `binding` value says which check a column actually got, and all five matter. `row-scoped` is the strong one: every row named a declared input and quoted that input's digest. `self-manifest` means every row named one of this receipt's own output artifacts instead — correct for a release manifest, wrong for a provenance ledger, which should be naming sources. `mixed` means both kinds of row are present in one column, which is legitimate only if the specification says that column carries both. `set-membership` and `partial` mean some or all rows named nothing this receipt declares, so those digests got only the weak test; ask why the rows name no source. Separately, a name listed under `ambiguous_input_names` or `ambiguous_artifact_names` was refused as a binding target because two declared inputs (or two artifacts) share it, so rows referring to it fell to the weak test through no fault of their own. Its `FAIL` findings never reach you — a failing candidate is re-fired before this audit — so what you receive is a passing report, and **a passing report is not evidence of derivation and does not discharge any question below.** Two parts of it are yours to adjudicate, because the guard cannot: its `WARN` findings (a column constant on every row, a declared column empty throughout, declared input files no row cites) are legitimate on an all-success outcome column or a deliberately filtered relation and are fabrication signatures otherwise, and only the specification settles which; and every column listed in `declared_derived_digest_columns` was exempted from digest membership by an orchestrator override that asserts the specification calls that column a derived digest — verify that assertion against the specification and FAIL an override it does not support.
...
6. **Check specification lineage and the evidence boundary** — reconcile code and report to every effective input/sample/variable/procedure/inference/output definition; verify each sample's requested-versus-observed period and key uniqueness, and each filter/merge/deduplication step's input/output count flow, units, and fingerprint; inspect every baseline deviation reason; and reject an undeclared or out-of-domain adaptive choice. Confirm every result has one analysis owner, every contract output is present, each exhibit's element map covers its result IDs, and comparisons are producer-computed results with comparable receipt-qualified operands whose receipt/bundle/artifact inputs are actually bound. Every paper-facing result must be in `RESULT_BUNDLE`; every result-bearing table/figure must be declared there and recreated by a renderer that sees only `renderer.inputs`. Flag access to audit-only summaries, raw inputs, recomputation, hard-coded numerical cells/series, undeclared exhibits, or report/bundle disagreement. Also check sample construction, missing-data handling, merge logic, standard-error specification, multiple-testing, economic significance, theory-match, identification, robustness, and overclaiming. Spot-recompute any non-headline number that looks suspicious; you are not required to recompute every result, only the ones that fail a plausibility check.
...
3. **Rerun from scratch (cache bypassed).** For every stochastic LOAD-BEARING spec, plus a sample of 2–3 stochastic STRENGTHENING-PROBE specs, recompute with the cache out of the way **in a private copy, never by touching the shared tree**: other auditors run concurrently and read the project's caches, so never move, delete, or write `data/` or any cache directory in place. Copy `code/` and the raw downloaded inputs the spec reads (never its derived cache) into a fresh `output/stage3a/audit_scratch/rerun_<spec>/` tree, preserving project-relative paths, and re-run the spec there in a fresh process with that tree as the working directory. Confirm the rerun wrote its derived cache inside the copy; a script that still reads the shared cache (a hard-coded absolute path) did not bypass it, so the check is void — report that as a finding rather than a match. Compare the recomputed value against the value reported in `ANALYSIS_PATH`. **Exact match required**: point estimates, standard errors, and CI/confidence bounds must agree bit-for-bit (floating-point ULP), because the fixed seed determines the draws. A non-zero delta is a determinism bug (an unseeded library call, dict/set iteration order leaking into the math, a `time`-derived seed), not a calibration choice → **FAIL**, with the offending call identified by file and line. Leave the copy under `output/stage3a/audit_scratch/`; nothing in the shared tree changed.
...
## Construction guard
| Warning or override | Specification says | Verdict |
|---------------------|--------------------|---------|
| [check + artifact + column from the report, one row each; write `none` if the report has no warnings and no overrides] | [the exact specification text that settles it, or `spec is silent`] | LEGITIMATE/ISSUE |

[A `spec is silent` row is itself a finding: a column the specification does not describe well enough to judge is a specification defect, not a clean bill of health.]
...
| analysis_id | deviations justified? | sample-step counts checked? | procedure/inference matches code? | result/exhibit ownership complete? | notes |
|-------------|-----------------------|-----------------------------|-----------------------------------|-----------------------------------|-------|
| [stable ID] | YES/NO | YES/NO | YES/NO | YES/NO | [details] |
...
- **Spot-check non-headline numbers as needed.** Write any verification scripts to `output/stage3a/audit_scratch/` — never beneath `code/`, whose every file enters the replication manifest, so a script there invalidates the freshness check and forces a full re-audit. Set random seeds. Scripts must be reproducible. You are not required to recompute every result — only the ones that fail a plausibility check.
- **Bypass the cache when testing reproducibility.** Re-running with the cache intact proves nothing — the cache returns the old number and hides any non-determinism. Re-run without the derived cache in a private copy under `output/stage3a/audit_scratch/` (never move or touch the shared cache — other auditors read it concurrently), in a fresh process, and demand a bit-for-bit match on every stochastic LOAD-BEARING spec. Exact-match is the standard: a non-zero delta on a non-allow-listed spec is a FAIL, not a tolerance call. This is separate from the headline-replicator's different-path recomputation — you test same-path determinism, it tests path-independence.
