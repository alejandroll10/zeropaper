...
<!-- DATA_FIRST_START -->
A revision earns score only when it adds new built evidence — a class or span built, a validation leg, a computed fact, an adjudication exhibit. Rewording, reorganization, label promotions or demotions, and restructuring around an existing result are typos — fix when wrong, but they do not move the score. See `docs/stage_4.md` for the catalogue and the orchestrator rule.
...
Stage 9: Polish               ──→ (parallel polish agents + triage + paper-writer in 2 sequential passes + style re-run; max 2 rounds)
...
**Fresh-theory identity reset (mandatory and atomic).** `theory_version` is only a within-attempt counter, and even `theory_attempt` can restart during Regeneration, so numeric equality alone cannot identify a theory across fresh starts. Whenever `theory_attempt` changes or `theory_version` is reset to 1 for a different theory, first retire every active Gate 3a-feasibility receipt belonging to the abandoned theory with a non-empty reason. Then, in the same `pipeline_state.json` update that starts the new theory, set every acceptance-version field present in this deployment to `null`: `stage2b_theory_version`, `stage2_mechanism_version`, `dataset_spec_version`, `stage2_design_version`, `stage3a_theory_version`, and `stage3b_theory_version`. Keep accepted report/receipt path pointers unchanged until their fresh cumulative replacements pass review. A null acceptance version prevents those old pointers from satisfying Gate 4 while preserving their active evidence for `--supersedes`. Every Gate-4 check requires equality against a non-null acceptance version. This reset applies to Regeneration, PIVOT, BACK-TO-IDEA, Gate-2 cap failure, Gate-3 KNOWN, Gate-3 INCREMENTAL cap, empirical-feasibility FALSIFIED, and any future fresh-attempt route; on an unseeded run, any of these routes except Regeneration that would raise `theory_attempt` above 5 takes the escalation table's "5 theories on same problem" row instead, whatever killed the theories; no caller may reset only the fields its current mode happens to consume. Loops do not self-zero here either. The generic artifact-scoped auto-reset is what normally clears them, and exception 3 below suppresses exactly that for episode-scoped loops, so in the same atomic update also set `loops.math_audit.round` and `loops.gate3_incremental.round` to 0 (base loops; each one's retry regenerates the draft it counts, so only its own clearing verdict or this reset clears it) and `loops.spec_audit_revision.round`, `loops.audit_fix.round`, `loops.unowned_failure.round`, and `loops.stage3a_upstream_return.round` to 0 — for whichever of those empirical-extension entries this deployment actually has, since those four exist only under `--ext empirical` (or on first reference); never create a missing one. `spec_audit_revision` is named for the same reason as `math_audit`: its own cap route is this reset, so a counter carried into the new theory would cap on its first REVISE. `audit_fix` and `unowned_failure` are named because they are the exception-3 loops whose own cap route **returns to Stage 2**: a counter carried into a brand-new theory would cap on that theory's first failure and send it straight back to Stage 2 with zero real attempts, a worse failure than the one each cap exists to prevent. `stage3a_upstream_return` is named because a fresh theory identity **is** its reset scope — it counts abandoned build campaigns within one `theory_attempt`, and every route that ends one arrives here. The other exception-3 loops need no entry here — `table_legibility`, `evidence`, and `polish` already reset on the fresh stage entry a new theory forces, and `last_resort_stuck` is deliberately scoped to the impasse rather than the theory. `loops.gate4_scorer_evaluations` is likewise **not** zeroed here: it is problem-scoped by design (it exists to count scorer spend across every theory tried on the problem), and its only reset write site is a `problem_attempt` change. Declaring this reset in a loop's registry reset scope is not enough on its own; this is its write site.
<!-- DATA_FIRST_START -->
Data-first never creates a Gate 3a-feasibility receipt, so the retirement step above is a no-op here. In the same atomic write, increment the run-global `dataset_acquisition_epoch`: banked Stage 3a acquisition units live under `output/stage3a/acquisition/e{dataset_acquisition_epoch}/`, so the new theory starts with no banked units while earlier epochs stay on disk as history, never consumed. In the same write also zero `loops.spec_audit_format.round` and `loops.coverage_certificate_producer.round`: both count retries of one Gate-2 firing, and a theory abandoned mid-retry (Gate 3 runs alongside Gate 2, so a KNOWN can end it there) must not hand a partial count to the next one. A data-first **PIVOT** is the one exception to the epoch increment and keeps the current epoch: its pivot object is the fact portfolio, not the dataset, so the banked units carry forward and Stage 3a's class-by-class plan diff decides which re-acquire. For data-first, the pointers preserved by this reset include `dataset_release_path` / `dataset_release_receipt`; the prior pair remains active only as the supersession predecessor until the fresh offline release replacement passes.
...
- **Two same-shape rounds bind early.** When a loop's last two rounds failed on identically-named findings with no measurable progress between them — the same items recurring with unchanged severity, count, and scope, or the numerical remainder the reports themselves treat as dominant staying flat — further rounds of the same shape are established as non-converging: treat the loop as capped now, whatever `round` reads, and take its FAIL route with both matching reports as the diagnostic. A repeated verdict *class* is never the trigger — a referee returning Major Revision twice over different comments is progressing; the trigger is the same named findings, quantitatively unmoved, and any change in an item's severity, count, or scope (or movement in the dominant remainder) counts as progress and defers the rule. Loops whose FAIL route is itself round-keyed are exempt — `referee` (the round-10 rows below) and `gate4_scorer_evaluations` (the plateau logic in `docs/stage_4.md`) already price repeated shapes at their own horizons. Seeded-mode negative-result budgets are *not* exempt: a round that leaves every finding unmoved is not one of the genuine attempts the ship-honest override budgets for. (Field evidence: a data-first run spent six consecutive repair laps against a flat remainder — 27,886 → 27,880 unmapped dependencies — then six more full-completion laps each failing independent review, the last two on identically named domains, and only an operator directive stopped it. Every lap after the second identical failure bought nothing; #338.)
- **A cap may not be changed while `loops.<id>.round` is within one of it.** A change takes effect at the next episode or after a reset. Sizing a cap is a build-versus-escalation cost judgement made in advance; raising one from the outcome of the attempts it has just counted is an outcome-level argument for removing the bound at the moment it is about to bind, and the cap route already carries the evidence that would justify a genuine change.
...
3. **Retry-regenerates-the-artifact non-reset** — a loop whose own retry *is* a regeneration of the artifact it counts, so artifact-scoped auto-reset would zero the counter on every iteration and defeat the cap. `loops.last_resort_stuck` is the reference case (see "a last resort for stubborn problems" above): it is scoped to the stuck *episode*, and resets only when the impasse clears or the loop is exited by certification.
   ...
   Stage 3a's `loops.audit_fix` is the third case, and the sharpest: an empirics-auditor FAIL routes through the fresh-attempt transition, so *every* round regenerates the analysis the counter is scoped to. Under artifact-scoped auto-reset the count would be zero on arrival at the cap check, forever. It is scoped instead to the build campaign, and resets on exactly two things: a candidate accepted end to end — the receipt activates and the `stage3a_analysis_path` handoff succeeds — and a change of `theory_version` or a fresh theory identity, which starts a different campaign and therefore a new scope. The second trigger is not optional: this loop's own cap route returns to Stage 2, so without it a capped counter would survive into the revised or brand-new theory's Stage 3a and terminate it on its first FAIL, with zero real attempts. An empirics-auditor PASS that is later retired on a downstream REVISE does **not** reset it: that candidate did not resolve the empirics-layer difficulty, and treating it as a reset is what lets a campaign run indefinitely by producing an occasional passing candidate that dies downstream. Neither a fresh attempt, a new analysis version, a restart, nor a context compaction resets it.

   Stage 3a's `loops.unowned_failure` is the fourth: it bounds the routings no other counter names, so every one of its retries is a fresh attempt regenerating the analysis, and its cap route returns to Stage 2 — neither an attempt nor a `theory_version` bump may zero it, or the cap self-feeds. A counter that is merely preserved or frozen does not own a routing (exception 1 is a rule about not zeroing, not a claim of ownership). Its registry row carries the scope; the operational two-question ownership test lives at `docs/stage_3a_empirical.md` "Unowned failures".

   Stage 3a's `loops.stage3a_upstream_return` is the fifth, and the ceiling over the other two: the `theory_version` bump is the very move it counts — each upstream return costs one respecification and buys a fresh build budget, so a per-version reset would let the design question be deferred indefinitely. It resets only on a candidate accepted end to end, in the same sense as `audit_fix` above, and on a fresh theory identity. Its registry row carries the scope; the increment rule and its cap route (`last-resort`, then the `branch-manager` certification bounded by `loops.last_resort_stuck`) live at `docs/stage_3a_empirical.md` "The upstream-return ceiling".

   `loops.math_audit` is the sixth: its FAIL retry is a theory-generator mutate that regenerates the very draft the auditors read next, so artifact-scoped auto-reset would zero it every cycle. Only a full Gate 2 PASS (structured and freeform) or the fresh-theory identity reset clears it.
   ...
   The same holds for every loop whose REVISE retry is a mutate or a fresh build attempt: `spec_audit_revision` (each REVISE or GAPS mutates the spec), `gate3_incremental` (each INCREMENTAL rework mutates the theory), and Stage 3a's `data_integrity` and `method_check` (each REVISE re-fires a fresh attempt). The regeneration their own route causes does not reset them. They reset where their docs say — on acceptance of the gated artifact, a different verdict class, or re-entry on upstream content revised by some *other* route — and on the fresh-theory identity reset.
...
<!-- DATA_FIRST_START -->
   The data-first `loops.plan_review` (each re-plan writes a new plan) and `loops.coverage_audit` (each REVISE re-fires a fresh attempt) follow the same rule; `plan_review` resets when step 4 approves a plan or Gate 2 accepts a new spec (a new spec starts a new campaign), `coverage_audit` on acceptance end to end or Gate-2 acceptance of a new spec. `loops.spec_audit_format` counts Gate-2 re-fires for malformed pre-launch artifacts or routing output and resets when a complete audit is routed.
...
**Loop Registry.** The complete set of capped loops. `cap` is the value seeded into `loops.<id>.cap`; the orchestrator reads the cap from state, never hard-codes it. Empirical-extension loops (marked †) are seeded at setup only under `--ext empirical`; a † loop that a mode consults without that extension (only `spec_audit_revision`, under measurement-first) is created on first reference with the default cap, as is any row marked "not pre-seeded in state".
...
| `math_audit` | 3 | current theory (`theory_attempt`) — **exception 3**: its own retry regenerates the draft it audits, so artifact-scoped auto-reset never zeroes it; it resets on a full Gate 2 PASS (structured **and** freeform — the freeform audit shares this cap, and if the structured PASS alone reset it, repeated freeform failures could never reach the cap) or on the fresh-theory identity reset | escalate to fresh theory attempt or sketch swap — `docs/stage_2.md` Gate 2 |
| `gate4_scorer_evaluations` | 8 | current problem (zeros only on a new `problem_attempt`; **survives fresh-theory resets and Regeneration** — it counts scorer launches across every theory tried on this problem, which is exactly what version-scoped `scores` keys cannot do) | the unseeded scorer hard ceiling: REVISE-band-or-above → deepening playbook; below → escalate one level — `docs/stage_4.md` |
...
| `reconcile` | 3 | current problem (not pre-seeded in state; created on first reference) | forbid further RECONCILE, default HONEST-NULL — `docs/stage_puzzle_triage.md` |
| `gate3_incremental` | 3 | current `theory_attempt`'s consecutive Gate-3 INCREMENTAL reworks (not pre-seeded in state; created on first reference; **exception 3** — resets on NOVEL or the fresh-theory identity reset) | abandon the idea, return to Stage 1 — escalation table, `docs/stage_2.md` Gate 3 |
...
| `audit_fix` † | 5 | current `theory_version`'s Stage 3a build, since the last candidate accepted end to end — **not** per attempt (exception 3 below) | step-7 FAIL branch: retire the pending receipt, return to Stage 2 — `docs/stage_3a_empirical.md` |
| `stage3a_upstream_return` † | 2 | current `theory_attempt`'s Stage 3a build campaigns abandoned upstream since the last candidate accepted end to end — **not** per `theory_version`, and the upstream return it counts does not reset it (exception 3 below) | `last-resort` on the build campaign, then the `branch-manager` ceiling certification that already bounds it — `docs/stage_3a_empirical.md` "The upstream-return ceiling" |
...
| `unowned_failure` † | 4 | current run of consecutive Stage 3a attempts lost to a routing no other counter advances — resets on an owned routing or a fresh theory identity, **not** per attempt and **not** per `theory_version` (exception 3 below) | retire every pending receipt, return to Stage 2 with the counts broken down and no kind-to-remedy mapping — `docs/stage_3a_empirical.md` "Unowned failures" |
| `spec_audit_revision` † | 3 | the current Stage-2 artifact's consecutive Gate-2 revisions — data-first spec mutations, empirical-first mechanism REVISEs, measurement-first design non-ACCEPTs (seeded under `--ext empirical`; created on first reference with the default cap elsewhere) | fresh theory identity / sketch-swap escalation — `docs/stage_2.md` |
<!-- DATA_FIRST_START -->
| `coverage_certificate_producer` † | 3 | current version's Gate-2 census producer firing (malformed output and ERROR retries both count) | halt `status=halted_coverage_certificate_invalid` (malformed output) or `status=halted_coverage_census_unreachable` (ERROR retries) — `docs/stage_2.md` |
| `coverage_audit` † | 3 | current Stage-3a validation-layer build | coverage FAIL route — `docs/stage_3a_empirical.md` step 7.6 |
| `plan_review` † | 3 | current Stage-3a campaign's plan (data-first; resets when step 4 approves a plan or Gate 2 accepts a new spec) | Gate 2 spec-audit re-fire as an upstream return — `docs/stage_3a_empirical.md` step 4 |
| `spec_audit_format` † | 3 | current Gate-2 firing's malformed pre-launch artifacts and routing output (data-first; not pre-seeded in state; resets when a complete audit is routed) | halt `status=halted_spec_audit_format` — `docs/stage_2.md` Gate 2 step 1 |
...
**`archived_best_scores`:** Fixed object mapping regeneration keys (`r1`, `r2`, …) to the best Gate 4 score achieved on the pre-regeneration paper when that round begins. It starts as `{}`; at Regeneration Round N write `archived_best_scores["rN"]` as the max over the `scores` entries whose `a{theory_attempt}_` prefix matches the archived paper's own theory attempt — never a bare max over all values, which would mix in scores earlier abandoned theories earned. Consumers in `docs/stage_1.md` step 2 read that exact entry to compare the regenerated attempt's eventual Gate 4 score against the archive; if the regenerated attempt does not strictly beat it, restore `paper_archive/r{N}/` and ship. Never add round-specific top-level state fields.
...
<!-- DATA_FIRST_START -->
   **Data-first release override:** any fresh empirical attempt that changes code, cached data, source mappings, schema, or a released fact must also rebuild the versioned dataset release through `docs/stage_3a_empirical.md` step 5.5. Do not activate `RESULT_RECEIPT` at this generic step. Obtain the full audit quartet on the paired analysis/release candidate, verify both receipts, then let Stage 3a step 7.6 activate both and atomically update the analysis triple plus `dataset_release_path` / `dataset_release_receipt` before retiring either predecessor. A prose-only edit that genuinely reuses unchanged active evidence and release bytes verifies both stored receipts and leaves both pointer sets untouched.
<!-- DATA_FIRST_END -->
<!-- EXT_EMPIRICAL_END -->
...
<!-- DATA_FIRST_START -->
When the dataset is sound but the paper is thin, deepen the dataset contribution, not a model. Pick 1-2 moves the scorer, self-attack, or referee feedback points at: extend coverage (a longer span, a new event class, a staged class brought in), add a validation leg (an independent second source for a waived class), add an adjudication target with its side-by-side construction isolation, compute a new fact the committed schema already supports, or add construction-sensitivity panels for the headline facts. Each move changes the spec: re-launch `theory-generator` in mutate mode, re-pass Gate 2 (Gate 3 only if the contribution's scope changes), then Stage 3, the Stage 3a re-fire, and Gate 4.
...
The "two same-shape rounds bind early" rule (Audit-loop scoping above) makes every round count in this table a **ceiling, not a guarantee**, except on the loops it exempts: a non-exempt loop resolves after two identically-shaped, unmoved rounds, routing to the same Action column early.
...
| Gate 3 novelty INCREMENTAL | `loops.gate3_incremental` at cap (3 rework attempts at Stage 2) | Abandon this idea, return to Stage 1 for a new one: increment `theory_attempt`, reset `theory_version` to 1, and apply the fresh-theory identity reset. **Seeded/faithful:** does not abandon the seed — Gate 3 seeded override in `docs/stage_2.md` supersedes this row. |
<!-- NO_MODE_START -->
...
| Empirics audit fails | 5 audit-fix attempts (cap from `stage_3a_empirical.md`) | Treat as theory-version failure — re-fire theory-generator (mechanism mode) with the audit notes as input. This row abandons a build campaign upstream, so it increments `loops.stage3a_upstream_return.round` first and stops firing at that loop's cap, which routes to the ceiling escalation instead — `docs/stage_3a_empirical.md` "The upstream-return ceiling". Without that ceiling this row is the mode's unbounded cycle: it bumps `theory_version` directly, which resets `audit_fix`, and no other loop counts it. |
<!-- EMPIRICAL_FIRST_END -->
...
| Unseeded scorer: hard ceiling | `loops.gate4_scorer_evaluations` at cap (8) — incremented once per unseeded Gate-4 scorer launch; problem-scoped, survives fresh-theory resets and Regeneration (Loop Registry) | If score is in the REVISE band or above for the current target tier (see `docs/stage_4.md`): switch to deepening playbook. Otherwise (MAJOR REWORK or ABANDON band): escalate one level. This ceiling is an escalation trigger, not a stop — it forces a strategy change, consistent with the no-deadline principle. |
