...
3. **Retry-regenerates-the-artifact non-reset** — a loop whose own retry *is* a regeneration of the artifact it counts, so artifact-scoped auto-reset would zero the counter on every iteration and defeat the cap. `loops.last_resort_stuck` is the reference case (see "a last resort for stubborn problems" above): it is scoped to the stuck *episode*, and resets only when the impasse clears or the loop is exited by certification.
   ...
   Stage 3a's `loops.unowned_failure` is the fourth, and it is the one that answers what bounds a routing no counter names. Every other counter in that stage is advanced by a specific verdict, so an attempt lost to something no verdict covers — a producer crash, a replay non-conformance, a capability the runner cannot grant, an interrupted-turn recovery, a branch that explicitly increments nothing — spent a full build that nothing bounded. It is defined by what the *other* counters do rather than by the shape of the failure: increment when the repair transition is entered, an attempt was lost, and no counter is *advancing* toward its cap or handing off to acceptance for that routing. A counter that is merely preserved or frozen does not own the routing; exception 1 above is a rule about not zeroing a counter, not a claim that a frozen one governs anything. It is scoped to the current run of consecutive unowned failures — an owned routing resets it — and to a fresh theory identity, and it is deliberately **not** reset by a `theory_version` bump, because its own cap route returns to Stage 2 and a per-version reset would make that cap self-feeding.

   Stage 3a's `loops.stage3a_upstream_return` is the fifth, and it is the ceiling over the other two. Both of those cap routes give up on the build and send the campaign upstream for a revision, as do several verdict-level routes in the same stage, and a revision is what starts the next campaign — so each cap costs one respecification and buys a fresh budget, and the design question the cap exists to force can be deferred indefinitely one revision at a time. This counter is scoped to the `theory_attempt` for exactly that reason: the `theory_version` bump is the move it counts, so it must not reset on one. It increments whenever a Stage 3a route abandons the build and asks for an upstream fix — whatever loop capped, and including the routes that escalate on a verdict rather than on a cap — and it resets on only two things — a candidate accepted end to end, in the same sense as `audit_fix` above (the activation and pointer handoff, never a step-7 PASS that dies downstream), and a fresh theory identity. Its own cap route is the one place in this family that does **not** return upstream: it hands the campaign to the `last-resort` escalation above, whose `branch-manager` certification owns the abandon-or-restructure decision and is itself bounded by `loops.last_resort_stuck`.
