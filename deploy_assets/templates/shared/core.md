...
The reset also preserves the run-global `dataset_spec_serial` and the accepted `dataset_rights_inventory` / `dataset_rights_inventory_sha256` binding. The nulled `dataset_spec_version` makes that old binding ineligible for a fresh build until Gate 2 accepts a newly allocated inventory; serial values are never decremented or reused.
<!-- DATA_FIRST_END -->
...
<!-- EMPIRICAL_FIRST_END -->
<!-- DATA_FIRST_START -->
**`dataset_spec_serial` / `dataset_rights_inventory` / `dataset_rights_inventory_sha256` / `dataset_spec_version`:** `dataset_spec_serial` is a run-global, monotonically increasing allocator for rights-inventory filenames; increment and commit it before every dataset-spec producer launch, and never reset or reuse it. `dataset_rights_inventory` and `dataset_rights_inventory_sha256` are the exact serial-qualified machine-readable inventory and its `sha256:<hex>` digest last accepted together with the prose specification by Gate 2. `dataset_spec_version` records the corresponding `theory_version`; Gate 4 requires non-null equality with the current version. Fresh-identity resets preserve the serial and accepted path/hash but null the version, so a stale inventory cannot authorize a new build and remains available only as immutable predecessor evidence. The trusted release runner requires the current path, hash, and version binding before execution and snapshots the inventory as an immutable producer input.
