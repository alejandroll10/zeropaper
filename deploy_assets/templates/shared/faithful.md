...
| Developing agent (empiricist/theory-generator/experiment-designer) derives a value different from the seed's stated result | `[DOCUMENT-AND-PROCEED]` + the existing owning correctness audit. The actual result enters the evidence artifact and paper once verified; document the discrepancy and honest framing in `limitations.md`. **Never replace or omit a verified finding to match the seed.** Fidelity fixes the research direction and contribution framing, not the answer. A failed correctness audit follows its normal `[FIX]` route; a verified prediction conflict follows emergent-headline / puzzle-triage / HONEST-NULL as applicable without suppressing the evidence. |
...
Every faithful-mode path that ends the run pending operator intervention — the **Gate 2** ship-honest failure (nothing shippable remains; the override file is shared with plain `--seed` and branches on `--faithful` internally) and **Gate 5 Reject** at the 2-rejection threshold — must set `pipeline_state.json:status = "halted_seed_abandon"` **in the same step**, leaving `current_stage` where it is. Commit the state change and report together.
