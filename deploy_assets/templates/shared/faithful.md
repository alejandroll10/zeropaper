...
- **Load-bearing structural invariants** — the features without which the mechanism is no longer the seed's mechanism. These are the things that, if dropped, would let an honest reader say "that's not the paper the seed described." Extract them by asking, for each named feature: would the seed's stated contribution survive removing this? If no, the feature is load-bearing. This is what the mechanism's structure must *contain*, never what it *predicts*: an existence claim, threshold, sign, or comparative static belongs to the excluded-predictions rule below, not here, even when the seed frames it as an essential feature.
- **Theorem-statement constraints** — what at least one theorem of the paper must contain (named free parameters, asymptotic regime, dependence structure, equilibrium concept) in order for the theorem to actually be about the seed's mechanism rather than a degenerate or simplified case. This is what the theorem's statement must *reference*, never what it *concludes*: a predicted sign, existence claim, threshold, or comparative static is a prediction and is excluded here even when the seed phrases it as structural.
...
- **Stated contribution / discovery claim** — what the seed claims to find or contribute. Demoting this (from "discovery" to "microfoundation for known facts," from "novel estimator" to "instance of an existing framework," from "causal effect" to "descriptive correlation") is a contribution-claim change and must be documented as a deviation, not silently absorbed. Also extract a single verbatim **`Headline:`** sentence — the one claim the abstract must still assert. All downstream contribution-drift checks compare against this sentence, not paraphrased prose. If the seed's own contribution sentence conflates the object studied with a predicted outcome, extract only the object-level clause ("we characterize how AI's cross-sectional coverage bounds its returns," not "we show the optimum is interior").

Claims about what the model will *yield* (a sign, a threshold, an interior optimum, a comparative static) are **predictions, not contract terms**: they are not extracted into the contract at all — they stay in the seed as hypotheses for the pipeline to test, and falsifying one is never a contract violation.
...
| Developing agent (empiricist/theory-generator) re-derives a value different from the seed's stated result | **Calibrated/estimated/empirical magnitudes only:** `[DOCUMENT-AND-PROCEED]` + log — the seed's stated value remains the contribution, the discrepancy goes to `limitations.md`, and this is **not** a correctness `[FIX]`. Two exclusions: a provably-false **theorem conclusion** is corrected via the math-auditor `[FIX]` row above and the corrected value enters the paper; a **prediction** (sign, existence claim, threshold, comparative static) that Step 0 excluded from the contract is never frozen here — a theory-generator divergence routes to the emergent-headline path (`docs/stage_2.md`, which is skipped under `--mode empirical-first`, so use HONEST-NULL there), an empiricist divergence to HONEST-NULL via puzzle-triage, and where neither gate applies the honest finding is reported to `output/seed/limitations.md`. |
...
4. **Assess maturity and enter the pipeline at the appropriate stage.** Populate all prior-stage artifacts (problem statement, selected idea, theory draft, etc.) needed to reach the entry point. The problem statement is written **directly from the contract** here — `question-poser` (Stage 0 Step 0d) and `question-referee` (Step 0e) are **not launched** in faithful mode; the contract defines the question, reproduced verbatim. Preserve the contract verbatim.
   ...
   **Stage 1 artifacts (if back-filled):** in faithful mode K=1, so: (a) set `loops.idea.round: 1`, (b) write `output/stage1/round_1/selected_idea_1.md` (for Gates 1b/1c), (c) write the canonical `output/stage1/selected_idea.md` (identical copy, for Stage 2). Add one entry `{round: 1, rank: 1, sketch_name: "<seed-descriptor>", novelty: null, prototype: null, reviewer_importance: null, eliminated: false, winner: true}` to `stage1_candidates` — the seed is winner by construction; runner-up-on-re-entry does not apply (so the `reviewer_importance` tiebreak axis is never consulted, hence `null`). The selected_idea.md must reproduce the contract's named mechanism and structural invariants verbatim — these are the load-bearing claims theory-generator will be required to honor. Leave its **Committed candidate answer** field as `none — answer emerges in development` and route any seed-stated predicted outcome into **Testable predictions** instead: a committed answer is a target theory-generator is instructed to *prove*, which would re-bind the prediction Step 0 excluded and disable Stage 2b's emergent-headline correction.

   **Stage 3 prerequisite for any entry at Stage 4 or later.** `output/stage3/implications.md` is a hard input to three downstream agents — the `scorer`'s Surprise cap/floor rules at Gate 4, `paper-writer` at Stage 5, and `puzzle-triager` — none of which has a fallback for its absence. A mature contract that back-fills its way past Stage 3 would reach them with no such file. So when the assessed entry point is Stage 4 or later, **run Stage 3 (`docs/stage_3_implications.md`) on the back-filled theory draft before transitioning `current_stage` past `stage_3`**; back-filling the artifact by hand is acceptable only if it carries real per-implication NOVEL / PUZZLE-CANDIDATE / SUPPORTED / DEAD lit-check tags, since those tags are what the consumers actually read.
...
Every faithful-mode path that ends the run pending operator intervention — the **Gate 2** ship-honest failure (nothing shippable remains; the override file is shared with plain `--seed` and branches on `--faithful` internally), **Gate 4 ABANDON**, and **Gate 5 Reject** at the 2-rejection threshold, all three of which write `output/seed/abandon_report.md`, plus the **branch-manager RESTART/REGENERATE** halt-and-escalate row in the routing table above, which does not — must set `pipeline_state.json:status = "halted_seed_abandon"` **in the same step**, leaving `current_stage` where it is. Where a report is written, commit the state change and the report together.

The report by itself is not a stop signal, and "halt and escalate to operator" is not one either. A resumed session routes on `status`, so a halt that writes only a file — or only prose in the commit log — is indistinguishable from a run interrupted mid-stage: the next session takes the resume path and re-enters the very stage the decision was meant to end. The token is what makes the decision terminal — the session entry point treats any `halted_*` status as operator-only and will not auto-advance.
