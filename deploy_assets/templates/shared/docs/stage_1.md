...
<!-- DATA_FIRST_START -->
   - In that same reset commit, also set `dataset_spec_version = null`; preserve the run-global `dataset_spec_serial` / `dataset_coverage_certificate_serial`, accepted `dataset_rights_inventory` / `dataset_rights_inventory_sha256`, and conditional `dataset_coverage_certificate` / `dataset_coverage_certificate_sha256` bindings.
<!-- DATA_FIRST_END -->
