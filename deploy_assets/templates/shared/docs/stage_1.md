...
2. **If a paper draft exists (i.e., post-Stage-5):** archive the current paper to `paper_archive/r{N}/` before generation begins, and record its best Gate-4 score at `archived_best_scores["rN"]` in pipeline state (defined as the max over the `scores` entries whose `a{theory_attempt}_` prefix matches the archived paper's own theory attempt, at archive time — never a bare `max(scores.values())`, which would mix in scores from earlier abandoned theories that the archived paper never earned; an approximate baseline either way, since Stage 6 referee revisions can improve the paper without producing a new numeric score). If the new attempt's eventual Gate 4 score does not strictly beat that entry, restore the archived paper and ship it.
...
<!-- DATA_FIRST_START -->
   - In that same reset commit, also set `dataset_spec_version = null` and increment `dataset_acquisition_epoch`; preserve the run-global `dataset_spec_serial` / `dataset_coverage_certificate_serial`, accepted `dataset_rights_inventory` / `dataset_rights_inventory_sha256`, and conditional `dataset_coverage_certificate` / `dataset_coverage_certificate_sha256` bindings.
<!-- DATA_FIRST_END -->
   - `triaged_lit_implications → []`
   - **Every `loops.<id>.round → 0` except `loops.stage0_discovery.round`, `loops.gate4_scorer_evaluations.round`, `loops.pivot.round`, and `loops.reconcile.round`** (leave each `loops.<id>.cap` unchanged; the last two are problem-scoped, and Regeneration keeps the problem). Regenerating the theory is a fresh artifact version for every downstream audit loop, so per the generic **Audit-loop scoping** rule (`CLAUDE.md`) those loops start clean; `stage0_discovery` is not an artifact audit but the run-global broad-scan budget and never resets, and `gate4_scorer_evaluations` is the problem-scoped scorer-spend counter that exists precisely to survive theory changes on the same problem (Loop Registry). This single instruction replaces the old hand-threaded per-counter reset list (`referee`, `reject_cosmetic`, `fix_empirics`, `headline_replication`, `data_integrity`, `method_check`, the claim loops, …). The regenerated paper thereby gets its own full Stage 6 `referee` budget and fresh empirical-audit budgets without reopening broad-scan capacity.
...
4. **Handle non-design outcomes.** If the designer returns `N/A — no causal claim` (the question is purely descriptive / calibration / model-fit despite empirical-first framing), `OUT-OF-SCOPE` (macro toolkit required), or `N/A — no design feasible from the available data variation`: do **not** route to `puzzle-triager` (its entry check requires `output/stage3/implications.md` and `output/stage3a/empirical_analysis.md`, neither of which exists yet). Instead, treat as a Stage-1 escalation:
   ...
   - Apply the recommendation as follows:
     - **REENTER-STAGE-1**: branch-manager named a runner-up sketch. Set `current_stage = "stage_1"`. In the same state commit, reset `loops.audit_fix.round`, `loops.unowned_failure.round`, and `loops.stage3a_upstream_return.round` to 0 for whichever of those entries this deployment has (all three exist only under `--ext empirical`; never create a missing one): the runner-up is a different idea, so the abandoned campaign's failure counts must not be spent against it, and these are `core.md` exception-3 loops the generic artifact-scoped rule deliberately does not reach. Unlike **REENTER-STAGE-0** below, this route changes no identity field, so nothing else resets them. Then apply the runner-up re-advance protocol in step 2 above (`Pre-screened runner-up`) using the named sketch. Commit `pipeline: stage 1 re-entry — empirical-first no-design escalation, advancing runner-up {sketch_name}`.
