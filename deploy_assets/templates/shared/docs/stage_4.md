...
<!-- EMPIRICAL_FIRST_END -->
<!-- DATA_FIRST_START -->
   - Empirics audit: `output/stage3a/empirics_audit.md` (H3 gates on this — includes the spec-conformance block)
   - Data-integrity audit: `output/stage3a/data_integrity_audit.md` (H3 gates on this — step 7.5)
   - Data-selection audit: `output/stage3a/data_selection_audit.md` (H3 gates on this — step 7.5)
   - Coverage audit: `output/stage3a/coverage_audit.md` (H3 gates on this — the triangulation verification)
   - Staged classes: if the spec carries a `## Construction staging` section, the accepted build report at `pipeline_state.json:stage3a_analysis_path` must list every staged class as `built in this report` in its `## Staged classes` section. A class still `outstanding` means the dataset is banked but incomplete — the follow-up campaign in `docs/stage_3a_empirical.md` step 7.6 has not run to acceptance. Do not score it as complete; flag it and route back.
   - Dataset spec audit: `output/stage2/mechanism_audit_v{N}.md` (the data-first Gate 2 verdict and REQUIRED / NOT-REQUIRED coverage-certificate decision). H3 gates on its **freshness**: the audit file for the version being scored must exist with a PLAUSIBLE verdict and `pipeline_state.json:dataset_spec_version` must equal `theory_version`. When REQUIRED, also read the exact certificate resolved from `pipeline_state.json:dataset_coverage_certificate`, verify its stored digest and current spec/rights bindings, and require `status: PASS`; when NOT-REQUIRED, both certificate fields must be null. If any condition is missing or stale, the spec gate was bypassed — do not advance; flag it and route back to Gate 2 (this mirrors the orchestrator's own Gate-4 enforcement and gives the scorer an independent second line on it).
   - (Math audit and theory exploration files do not exist under data-first; do NOT attempt to read `output/stage2/math_audit_*.md` or `output/stage2b/exploration.md`. The identification audit does not exist either — the identification agents are pruned in this mode.)
<!-- DATA_FIRST_END -->
<!-- MEASUREMENT_FIRST_START -->
   - Design review: `output/stage2/design_review_v{N}.md` (the plan-time Gate 2 verdict; H3 gates on it, together with the math audits above — which fire on the post-experiment characterization in this mode — and the Stage 3b chain). **Confirm the gate is current**: `pipeline_state.json:stage2_design_version` must equal `theory_version`; if missing or stale, the design gate was bypassed — do not advance; flag it and route back to Gate 2.
...
<!-- NO_MODE_END -->
<!-- MEASUREMENT_FIRST_START -->
   - **Structured scorer only, on revisions (N ≥ 2):** also pass the prior theory draft (`output/stage2/theory_draft_v{N-1}.md`) and, **only if it exists**, the `## Unverified claims` section of the prior math audit (`output/stage2/math_audit_v{N-1}.md`) — under measurement-first, audit files exist only for characterization versions, so a construct-spec v{N-1} has none; pass only the prior draft then. Do NOT pass any prior scorer output or prior score to either agent — the structured scorer scores independently; the freeform scorer is history-blind.
<!-- MEASUREMENT_FIRST_END -->
<!-- EMPIRICAL_FIRST_START -->
...
<!-- EMPIRICAL_FIRST_END -->
<!-- DATA_FIRST_START -->
   - **Structured scorer only, on revisions (N ≥ 2):** also pass the prior theory draft (`output/stage2/theory_draft_v{N-1}.md` — the prior spec version) and the prior empirics, data-integrity, data-selection, and coverage audit reports if they were re-fired alongside the spec revision. (Data-first has no math audit's `## Unverified claims` section to read; the analogous "what was unresolved last round" content lives in the prior audit REVISE notes from `output/stage3a/` and the prior spec audits under `output/stage2/mechanism_audit_v*.md`.) Do NOT pass any prior scorer output or prior score to either agent.
<!-- DATA_FIRST_END -->
