...
<!-- DATA_FIRST_START -->
2.5. **Allocate the rights-inventory identity before launch.** Increment the run-global, never-reset `pipeline_state.json:dataset_spec_serial`, commit that state change, and set `RIGHTS_INVENTORY_PATH = output/stage2/source_rights_s{dataset_spec_serial}_vN.json` using the newly committed serial and current N. Supply this exact path to `theory-generator`. A crash or failed firing consumes the serial; never decrement or reuse it. This allocation, rather than `theory_attempt` or `problem_attempt`, prevents a fresh problem/regeneration from overwriting a rights file still fingerprinted by an active release.
<!-- DATA_FIRST_END -->
...
<!-- DATA_FIRST_START -->
   In data-first mode the same firing must also save the preallocated exact `RIGHTS_INVENTORY_PATH` using the schema in the agent body. Treat the prose spec and rights JSON as one versioned artifact: never carry a prior serial's JSON forward, never synthesize it on the orchestrator's behalf, and never commit only one half.
<!-- DATA_FIRST_END -->
...
In data-first mode `theory-generator` runs in **dataset-spec mode** and produces two binding artifacts at the same version: the prose dataset specification `output/stage2/theory_draft_vN.md` and its machine-readable rights inventory `output/stage2/source_rights_s{dataset_spec_serial}_vN.json` — schema, dating conventions, inclusion/reconciliation rules, validation plan, source IDs plus redistribution evidence, and fact-portfolio plan. There are no derivations, so `math-auditor` / `math-auditor-freeform` have nothing to re-derive and are **not** launched. But a spec can be broken before any build: an inclusion rule a third party cannot operationalize, a "triangulation" whose second source is a mirror of the first, an `open` rights classification resting on assumption, a mismatch between the prose inventory and the enforcement JSON, a replication target with no expected value, a coverage claim the Stage 1 pilot contradicted. Those defects do not need a build to detect; catching them now costs one read, catching them at Stage 3a costs a full build against a broken spec.
...
1. Launch `mechanism-auditor` with explicit paths (the body has no hardcoded defaults): the dataset specification `output/stage2/theory_draft_vN.md`, its rights inventory `output/stage2/source_rights_s{dataset_spec_serial}_vN.json`, the pilot-build report `output/stage1/idea_prototype.md`, the problem statement `output/stage0/problem_statement.md`, and — **only if construction results already exist** (a mutate/pivot re-fire after Stage 3a) — the exact build report at `pipeline_state.json:stage3a_analysis_path` for the build-anchored coverage-count check. Name the output path `output/stage2/mechanism_audit_vN.md`. Before launch, parse the JSON and require `schema_version == 1` and `dataset_version == theory_version`; malformed, missing, or wrong-version JSON re-fires `theory-generator` without consuming an audit verdict.
2. Save result to `output/stage2/mechanism_audit_vN.md`.
3. Commit: `artifact: dataset spec audit v{N} — {PLAUSIBLE/REVISE}`.
4. Route:
   - **PLAUSIBLE:** only after the audit explicitly confirms that the exact serial-qualified rights JSON matches the prose inventory, compute its lowercase SHA-256 and atomically set `pipeline_state.json:dataset_spec_version = theory_version`, `dataset_rights_inventory = RIGHTS_INVENTORY_PATH`, and `dataset_rights_inventory_sha256 = "sha256:<64 lowercase hex>"`; reset `loops.spec_audit_revision.round` to 0, then proceed to Gate 3 (novelty check). A failed audit never changes the accepted path/hash binding.
