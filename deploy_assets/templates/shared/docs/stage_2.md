...
1. On the first-ever Stage 2b run (no `stage2b_result_receipt` pointer), set `EXPLORATION_PATH = output/stage2b/exploration.md`, `RESULT_PLAN = output/stage2b/results.plan.json`, `RESULT_BUNDLE = output/stage2b/results.json`, `RESULT_RECEIPT = output/stage2b/results.receipt.json`, `ANALYSIS_ENTRYPOINT = code/explore/run_all.py`, `RENDER_ENTRYPOINT = code/explore/render_exhibits.py`, `INPUT_SNAPSHOT_DIR = output/stage2b/inputs_a1`, and `SUPERSEDES_ARGS = ()`. Copy the theory draft, math-audit results, and data-inventory document into that fresh directory before writing `RESULT_PLAN`; launch `theory-explorer` on those copies and supply all seven exact paths plus the empty array. If a retained pointer exists with a null/stale acceptance version after a theory-identity reset, skip these canonical paths and use step 5's fresh cumulative-replacement namespace and supersession array.
