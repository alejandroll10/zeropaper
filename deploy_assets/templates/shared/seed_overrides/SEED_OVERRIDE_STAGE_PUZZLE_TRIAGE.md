...
- **RECONCILE**: unchanged — adding scope conditions preserves the seed. The `loops.reconcile` cap still applies; at the cap it routes to HONEST-NULL.
- **PIVOT**: **allowed and encouraged**. The PIVOT strategy preserves the original theory as a nested / baseline case — it does NOT abandon the seed. The original theory becomes the "what naive intuition would predict" baseline, and the pivoted theory explains the contradiction. Note the pivot in `output/seed/pivot_note.md`. Normal 2-pivot cap still applies.
