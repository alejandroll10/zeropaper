...
- **Don't fix a blocked idea.** If the claim doesn't survive the four screens (sign, magnitude, identification plausibility, data), report the failure and stop. Fixing is the idea-generator's job (or, if the block is a proven impossibility, the idea gets killed).
