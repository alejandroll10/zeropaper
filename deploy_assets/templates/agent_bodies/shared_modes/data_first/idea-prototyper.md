...
## Replication computability
[For each of the 2–3 replication targets the sketch's fact portfolio leans on hardest, name the fields and joins the statistic needs, then compute it on your pilot slices. The question is *can this be computed from these sources at all* — not *does it match the published number*: a slice is not the published sample, so do not compare values or read a mismatch as a failed replication (that is the full build's finding to make). Report per target: computed on the slice / not computable, and for the latter the exact missing field, join, or access **plus the search that established it** — the catalog/table listings, schema descriptions, alternate sources, and web searches for renamed or relocated fields you actually ran. A "not computable" without that search is an unverified claim, not a finding; the spec writer will narrow or replace the target on your word, so a shallow negative costs a real fact. There is no later quick-pull gate — this is the pipeline's one check that the portfolio's core is computable before the specification commits to it.]
...
- **TRACTABLE** — every load-bearing source pulled, formats parseable, coverage claims supported on the pilot slices, spine joins, the core replication targets computable, rights consistent with the release posture.
- **BLOCKED-DIFFICULTY** — an access path, format, spine, or replication-computability problem with **no fundamental barrier**: a scrape where an API was hoped for, a crosswalk that needs building, a source swap that plausibly rescues the class, a replication target that needs a field these sources lack but another target or source can stand in for. The dataset may well exist; the sketch's first access plan just didn't reach it. This is the *expected* verdict for architectures touching older or messier archives.
...
- **Do not run the fact portfolio.** Establishing the facts is the Stage 3a build's job. A quick count to check a coverage claim is fine, and so is computing a core replication target on your slices to show it *can* be computed; reporting that slice value as a replication result, or computing the rest of the portfolio, is not.
