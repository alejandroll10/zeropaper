...
- **Executability against the trusted runner is part of this dimension (issue #307).** A trusted run plan declares inputs only as regular files, each bound read-only and content-hashed, plus the analysis run's `network_access: true` (IP egress: FRED, EDGAR, and other HTTP clients need nothing more) and its `live_services`, the host services the runner binds by name — today only `wrds`, the daemon behind `wrds_query()`. The spec's `## Trusted-run live services` line is that list, and it has already been checked against the installed runner before your launch, so every name on it is one the runner provides. Two things remain yours. First, a non-empty line must name the cross-unit obligation that has to query the service live inside the trusted run and say why a banked pull cannot serve it: core-class acquisition belongs in the banked acquisition units, which run before the trusted run, so a line naming a service for acquisition is REVISE, back to `[]`. Second, the line is the spec's entire contract about live channels. A spec requiring the daemon's raw AF_UNIX socket, a PID or lock file, a credential relay, or any other non-regular-file primitive to be declared as an input, hashed into a sandbox policy, or reconciled in observed I/O is unexecutable, and without this check the conflict surfaces only at receipt publication, one full build later, on every attempt. (Field evidence: one campaign spent five consecutive attempts on exactly such a spec-required socket binding; the runner correctly failed closed each time.) REVISE, naming the correction: a runner-provided service goes on the line, which is the binding, since the runner binds exactly the declared services and fingerprints the declaring plan into the receipt; anything else is pulled beforehand as ordinary work, banked, and declared as the cached file.
