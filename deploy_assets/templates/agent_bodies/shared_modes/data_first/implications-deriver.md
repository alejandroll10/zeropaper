...
- **Executable on the committed schema.** Every test design hint must run on columns and classes the spec actually defines — an item consuming data the schema lacks belongs in a final "Unsupported by committed schema" section rather than silently dropped (the Stage 3a plan decides whether it is a spec gap).
