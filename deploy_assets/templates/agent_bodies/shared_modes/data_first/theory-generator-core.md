...
- **Never require the runner to bind a non-file primitive.** Trusted run-plan inputs are regular files, hashed; the networked analysis run additionally declares IP networking, and the standard clients (`wrds_query()`, FRED, EDGAR) already work inside it on that capability alone — specify pulls through them freely, with no spec-level binding. Do not write a spec that demands the WRDS daemon's raw AF_UNIX socket, a PID or lock file, or any other non-regular-file primitive be declared, bound, or hashed into a trusted run — the runner structurally cannot do it and Gate 2 REVISEs it (issue #307). If an audit thread asks for such a binding, the answer is this rule, not compliance: rely on the declared network capability, or bank a pre-run pull and declare the cached file.
