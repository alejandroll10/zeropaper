...
## Trusted-run live services
[Begin with exactly `**Live services:** []`: one sorted JSON array naming the host services the single trusted analysis run reaches beyond ordinary IP egress. Every name must be one the installed runner provides — `python3 code/utils/results_pipeline/results_pipeline.py live-services` lists them (today only `wrds`), and Gate 2 checks the line against that command before the audit launches. The default and usual value is `[]`: core-class acquisition runs in the banked acquisition units before the trusted run, which reads their caches and re-pulls nothing. Name a service only when a cross-unit obligation genuinely must query it live inside the run, and say which obligation and why a banked pull cannot serve it. FRED, EDGAR, and other HTTP sources are ordinary network access and never appear here, and the offline release run declares none. This line is the spec's entire contract with the runner about live channels: the runner binds exactly the named services and nothing else, and the plan declaring them is fingerprinted into the receipt, so nothing further about a channel is declared, bound, or hashed anywhere in the spec.]
...
- **Science only, no build apparatus.** Specify what the dataset is: sources, schema, rules, validation, rights, coverage, portfolio. Never specify how the build proves itself. The pipeline already supplies provenance: the trusted runner content-hashes every input and output into a receipt, the construction guard checks derivation, and four Stage 3a auditors re-query the sources. So the spec carries no I/O tracing or replay layers, sandbox grammars, byte-locator or hash-binding contracts (beyond the rights inventory), admission contracts, fixture graphs, static closure proofs, review gates of its own, or pinned bespoke runtimes, and Gate 2 REVISEs any it finds. Estimators use the maintained packages in the provisioned environment (read `.claude/skills/canonical-packages/SKILL.md` before naming one). If an audit thread asks for machinery, the answer is this rule, not compliance. The spec's reply to any concern about the build's execution is a clearer rule or a stronger validation leg.
- **Live channels go on the `**Live services:**` line, nowhere else.** Trusted run-plan inputs are regular files, hashed; the networked analysis run adds IP egress, and a host service such as the WRDS daemon is reached only when `## Trusted-run live services` names it — the runner then binds that service and records the declaration. Do not write a spec that demands the daemon's raw AF_UNIX socket, a PID or lock file, a credential relay, or any other non-regular-file primitive be declared as an input, bound, hashed, or reconciled in any other way: the runner cannot, and Gate 2 REVISEs it (issue #307). If an audit thread asks for such a binding, the answer is this rule, not compliance: the service name on the line *is* the binding; otherwise bank a pre-run pull and declare the cached file.
