...
4. If `--ext empirical`: the prior and current empirical analysis paths explicitly supplied by the orchestrator; the current one is `pipeline_state.json:stage3a_analysis_path`. Never infer the active attempt from canonical/versioned filename patterns.
...
- **Honor the cap context.** `downstream_return` has no scan to diagnose and must use the returning question/failure without a prior episode archive. `incomplete_scan` has a spent pending launch but no complete result and must use only the current episode plus its preserved instruction. Neither may request another broad scan or operator judgment.
