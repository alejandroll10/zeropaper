...
- On revisions (N ≥ 2): the prior theory draft. Do NOT read prior scorer decision files — your value is an independent, unanchored score. Score this version on its own merits.
<!-- THEORY_FIRST_START -->
...
Before scoring, check whether the self-attack report contains (a) an explicit `**Load-bearing premise:** …` line drawn from the theory draft's stated assumptions / data / methodology (not a strawman invented for the report — you are already reading the theory draft, so judge anchoring here), and (b) at least one Assumption-attack group targeting that premise. If any of these is missing, prepend a `Self-attack quality:` line to your output recording the gap (e.g., `Self-attack quality: load-bearing premise unnamed — Completeness/robustness-only scrutiny; treat severity ≥ 7 robustness attacks with skepticism per self-attacker rule`) and proceed to score. Do NOT return REVISE solely on this basis — routing REVISE here would loop against theory-generator, which cannot fix a self-attacker's premise coverage. The orchestrator's Stage 4 step 3a re-fires the self-attacker only when the premise **header** is absent; a header that is present but unanchored or unattacked is caught only here, as this advisory flag (which softens branch-manager's read of robustness-heavy reports), not by a re-fire.
...
<!-- EMPIRICAL_FIRST_END -->
<!-- DATA_FIRST_START -->
**Data-first supersedes the own-design-critique guard's dataset clause.** Under `--mode data-first` the dataset release IS the contracted contribution class — the "dataset/pipeline release where the release itself is the claimed contribution" trigger in the guard above does **not** apply, and none of its exceptions (a)–(c) is needed for the release itself. The guard's *other* triggers stand unchanged: a data-first paper whose main contribution has drifted to a methodological warning, measurement caveat, or standard-error correction about its own pipeline still fails H1/H5 on those grounds. The operative contribution-class rule for this mode is the fidelity block below.

**Dataset-contribution fidelity (data-first only).** Data-first commits the paper to the dataset + fact portfolio in `output/stage2/theory_draft_vN.md` (the specification). Drift to a different contribution class — a causal-identification paper riding on the assembled data, a theory paper the data merely illustrates — is itself an H1/H5 FAIL: data-first chose the dataset as the contract, and a causal headline in particular is a claim this pipeline pruned its identification machinery for and cannot defend. Exception (c) applies identically in both blocks: a stated theorem with proof, applicable beyond this paper's data, is a legitimate contribution even under data-first. A puzzle-triager PIVOT that promotes a failed replication to a headline adjudication is NOT drift — the adjudication is a documented fact about data construction, squarely inside the contribution class.
<!-- DATA_FIRST_END -->
...
**You must identify, in one sentence, what {{IMPORTANCE_OUTCOME}} or belief this result would change if true.** If no specific decision or belief can be named, the score is below 55 regardless of how ambitiously the paper is framed. Framing cannot substitute for operational consequence. **For a multi-implication / applied-theory paper** (several co-equal results unified by one mechanism, *or* by {{> policy_map_axes }}), evaluate Importance on the *union* of headline findings — the consequence of the whole framework — not a single sub-result, and do not anchor on the weakest implication. For such a paper a margin is load-bearing when dropping it would reverse or materially change the framework's {{POLICY_MAP_LOAD_TEST}}; penalize only when margins are unrelated or decorative, or when the paper falsely claims a single mechanism delivers all of them.
...
- **Field-prior anchor (the external, literature-grounded surprise anchor).** Surprise is *the result vs. the field's prior*, not the result vs. any author prediction. Anchor it externally: read the novelty-checker's report (`output/stage2/novelty_check_v*.md`) and the literature map for the field's documented expectation on this question — the standard sign, the consensus magnitude, the accepted explanation. Then:
  - The developed result **overturns** a documented field prior (a sign the literature did not expect, a magnitude that contradicts the consensus, a mechanism that displaces the accepted explanation, or a framing the field had not applied to the problem) → strong evidence for a high Surprise score, even if the result reads as "clean." For the framing/category axis, where the lit map may carry no standard sign or magnitude to overturn, anchor on the non-obviousness fallback below.
  - The developed result **confirms** the field's prior → low Surprise however polished the delivery.
  - **Fallback when no directional prior exists** (existence / characterization / pure-structure results, where the field has no standard sign or magnitude to overturn): score Surprise on **non-obviousness** — could a {{SURPRISE_READER}} have called the result before the work? — together with the Cap-30 / Floor-70 rules below. This is the same fallback the question-referee's "not-obvious" axis used at Gate 0, now applied to the *delivered* result.
...
- **Implication-tag check** (`output/stage3/implications.md` — always present when you run; every path that reaches Gate 4 has passed Stage 3):
...
Parsimony is measured relative to the paper's core result: how many of the assumptions and model elements **in the core derivation of the main result** are load-bearing, versus added for scope or defense? Count assumptions and frictions, not implications — one friction that yields many implications is parsimonious, not a violation. **Parsimony scores the leanness of the core model only.** Robustness checks, extension appendices, alternative specifications, additional empirical analyses, and "we also show" results presented as separate extensions do **NOT** count against Parsimony — a paper is never penalized for being thorough or for relegating extra material to an appendix. The only thing that lowers Parsimony is a *core model* that carries more than its main result requires.
