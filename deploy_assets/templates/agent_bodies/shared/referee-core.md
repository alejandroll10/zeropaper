...
**Where the novelty sits is not a structural ceiling.** A structural ceiling is about the *magnitude* of the contribution, not about which literature its novelty belongs to. That the paper's domain layer is applied, decorative, or separable — and the genuine innovation lives in an adjacent literature ({{REFEREE_ADJACENT_LIT_EXAMPLE}}) — is **not** a structural ceiling when the paper is within the variant's domain scope (see the Variant context at the bottom of this body for the domain's sufficient conditions); top journals in the field routinely publish such papers on quality ({{REFEREE_CROSS_LIT_PRECEDENT}}). Judge importance, novelty, rigor, and surprise *wherever the novelty lives*; "the domain contribution isn't the central innovation" is not a reason the paper sits below the tier. What remains a valid ceiling is a genuine contribution-magnitude shortfall — the central contribution, wherever it sits, is too thin or incremental for this tier.
...
Write the report in this exact structure:{{REFEREE_VERDICT_NOTE}}
...
[Required only if the recommendation is Reject; omit this section otherwise. Describe the type of paper — keeping the current core idea — that would have a good chance of clearing this journal's bar. Be specific: which result should be the centerpiece, what additional {{REFEREE_DISCIPLINE_PHRASE}} would discipline the claim, what the headline contribution would look like.]
```
