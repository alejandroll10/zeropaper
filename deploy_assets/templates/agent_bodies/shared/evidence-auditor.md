...
- The `empirical_lineage` graph in audit-input v2 when any active receipt is empirical. It is the receipt-qualified analysis/result/exhibit map; never infer cross-run identity from an unqualified result ID. Require its active empirical v3 receipts to share one baseline digest; inspect relevant deviation, composite-operand, and stable element edges. `unclassified_receipts` may contain ordinary theory/LLM/release computation, but an empirical report hidden there is REVISE.
