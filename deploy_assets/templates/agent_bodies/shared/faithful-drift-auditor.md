...
2. The gate-specific contribution artifact the orchestrator must point you to explicitly:
   - **Gate 4:** the current theory draft, `output/stage2/theory_draft_vN.md` for the `theory_version` in `process_log/pipeline_state.json`, plus every current contribution-bearing evidence result that exists: the exact `stage3a_analysis_path` and any prior Stage 3a reports still needed for combined coverage; the exact `stage3b_results_path` and any prior Stage 3b reports still needed for combined coverage. Audit the draft's stated headline/main contribution **and each evidence artifact's own headline/contribution claims, named mechanism, and seed-fixed quantitative results**. The orchestrator must resolve and name the applicable paths explicitly from pipeline state; do not substitute a guessed canonical sibling, scorer, triage, audit, or self-attack report merely because it is newer.
