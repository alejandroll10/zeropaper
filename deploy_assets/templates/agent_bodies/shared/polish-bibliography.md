...
2. Verify every occurrence freshly at every checkpoint, including unchanged or moved uses. A project-local prior receipt cannot authenticate its own citation characterization, so prior summaries are context only and never authority. Set every entry to `"verification": "fresh"`.
3. Freshly verify every use against the cited paper itself, OpenAlex's abstract/metadata, or an authoritative full abstract/working-paper page found via WebSearch. Record an exact `https://`/`http://` primary or OpenAlex URL, `doi:10...`, or `openalex:W...` pointer for every key. A literature map, producer report, prior derived prose, or model memory is not a source. There is **no 50-lookup cap in checkpoint mode**: every use must be verified or the verdict is REVISE.
