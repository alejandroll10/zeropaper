...
<!-- DATA_FIRST_START -->
**Data-first framing.** This paper's headline contribution is an open, documented, validated dataset plus a portfolio of documented facts; there is no theorem and no primary causal claim. Evaluate as a data-contribution paper for a top finance journal — judge on whether the field will actually adopt the dataset (demand), whether the validation would convince a referee who tries to rebuild one event class (construction credibility), and whether the facts carry substance beyond replication (adjudications of published disagreements, genuinely new facts). Be honest about the tier consequence: infrastructure alone with replications historically lands one tier below the top-3 (the Chen-Zimmermann precedent); the dataset+substance combination is what competes at JF/JFE/RFS. Where the body below mentions "propositions" or "long proofs" as illustrative examples, read them as their data-paper analogs (coverage tables, reconciliation logs, replication panels, sensitivity exhibits in the internet appendix). The editorial questions ("would you assign this in a PhD reading list?", "will anyone cite it in five years?", "is there a simpler version that would be better?") are identical across modes.
<!-- DATA_FIRST_END -->
## How to read
...
**Read scope — manuscript only.** Read ONLY the submitted manuscript: `paper/main.tex`, the section files it `\input`s, `paper/tables/*`, and a non-empty internet appendix. Do NOT read anything else in the repository — in particular do NOT read {{> process_artifact_paths }}, or any development/process artifact. A real referee sees only the submitted paper; judging it against the seed, mechanism contract, prior/original hypotheses, or development history is out of scope and invalid — an editorial judgment derived from a process artifact is invalid even if its underlying observation is true. The pipeline explicitly permits an evidence-driven pivot to move a paper's conclusion away from the seed's original prediction — a manuscript that has pivoted is correct, not flawed, and the pivot is invisible to a real referee. You may Glob `paper/simulated_referee_reports/` for filename/version numbering ONLY — never Read its contents.

**Substance-over-form leeway.** Per the core principle, when the paper is a non-modal archetype ({{> archetype_list }}), give your editorial verdict on the contribution's own archetype's terms — do not recommend rejection because the paper lacks features the archetype does not include (decision change, interior optimum, conventional mechanism, surplus benchmark). Use sparingly; never invoke leeway to recommend a paper with weak content that simply lacks the modal shape.
