...
1. **Severity-≥7 default (Gate 4 only).** Any self-attack concern in the `### Severity 10` or `### Severity 7-9` sections of the input file defaults to `[FIX]`. Escape to `[LIMITS]`/`[RESPONSE]`/`[NOTE]` is allowed only with a one-sentence written justification naming the specific cost of the FIX and why it is unjustified. If you cannot write such a justification, leave the classification as `[FIX]`. **The self-attacker groups attacks by target (a specific model object) with a root attack and variants.** Treat each Target group as one concern row in the triage table — the group's severity is the max across variants, and the `[FIX]` applies to the group, not to individual variants. **Archetype-mismatch escape:** an attack whose force depends on the paper being a different archetype than it is ({{> archetype_list }} — consistent with the self-attacker's substance-over-form leeway principle) is downgradable to `[NOTE]` with a one-sentence justification naming the archetype and the attack's archetype assumption. Never invoke this escape to downgrade attacks on actually-broken content.
...
3. **Mechanism lockout (Gate 5 only).** If the mechanism referee's verdict is MECHANISM-MISATTRIBUTED or MECHANISM-DECORATIVE, every `[FIX]` in the mechanism report is **locked** — it cannot be downgraded. Record the verdict at the top of the triage file. A MISATTRIBUTED verdict means the paper's claimed driver is not its actual driver — the fix is rewriting around the actual driver. A DECORATIVE verdict means the mechanism is window dressing on a structural identity — the fix is either restructuring the paper to find real {{MECHANISM_QUALIFIER}} content (deepening-playbook path) or narrowing the claim to match what the math actually supports (scientist-first narrow path). Both verdicts route through Major Revision (never Reject), so triage *is* reached. Downgrading mechanism `[FIX]` items under these verdicts would silently ship a paper whose {{MECHANISM_QUALIFIER}} claim does not match its math. Rule 2's downgrade-with-justification escape does not apply to mechanism `[FIX]` items under these two verdicts; under MECHANISM-PARTIAL or MECHANISM-VALID, mechanism `[FIX]` items follow rule 2 normally.

3a. **Reject deepen-directive extraction (Gate 5 only).** Fires only when **the editor's aggregated verdict in `editor_decision_rN.md` is Reject** (i.e., the editor did not invoke the Rule 2 tier-fit escape). The editor already encodes the mechanism-override rule and the tier-fit-escape rule in its aggregated verdict, so triager rule 3a does not need to re-derive them — read the editor's `## Aggregated verdict` field. When the rule fires, locate each rejecting referee's `## What would be publishable` section (required by `referee-core.md` and `referee-freeform.md` on Reject) in the raw referee reports and copy it verbatim into a `## Deepen directive (Reject)` block as the first `##`-level block in the triage file (after the header fields). Tag the source referee. If both referees recommend Reject, copy both sections under sub-headers `### From structured referee` / `### From freeform referee`. If a referee recommends Reject but did not produce a `## What would be publishable` section, record this as `### From [referee] — directive missing` and flag in Summary as a triage error (the orchestrator must re-launch that referee, see `docs/stage_6.md` Reject row). The deepen directive is the canonical input for theory-generator / empiricist on the next round; the `[FIX]` table still records all referee comments but **on a Reject round, the `[FIX]` items are deferred — recovered by re-detection on the next round's referees, not by an explicit queue**. Note the count in the Summary block as `[FIX]-deferred: N (Reject deepen-directive applies; recovery by re-detection on next round)`. Mechanism referee never recommends Reject (its verdicts force Major Revision via the editor); rule 3a does not fire on mechanism referee. **Do NOT fire rule 3a when the editor's aggregated verdict is Major Revision (even if the editor's `## Aggregated verdict` justification quotes a Rule 2 tier-fit escape from a raw referee Reject vote).** **Citation hygiene on the verbatim copy:** the rejecting referee's "What would be publishable" section is one of the few places triager copies referee prose verbatim into a downstream-actionable artifact. Apply the same hygiene the editor applies to canonical-list comments (Rule 6 of `editor.md`): regex-scan the copied text for author-year patterns including `Surname (YYYY)`, `Surname and Surname (YYYY)`, `Surname, Surname, and Surname (YYYY)`, `Surname et al. (YYYY)`, disambiguated years `Surname (YYYYa/YYYYb)` and `Surname (YYYY, YYYY)`, parenthetical forms `(Surname, YYYY)` / `(Surname et al., YYYY)`, no-paren `Surname, YYYY` / `Surname YYYY`, and year-stand-ins `Surname (forthcoming)` / `Surname (in press)`. For each match, check for a trailing `[openalex:W…]` or `[doi:10.…]` tag within ~30 chars. **False-positive carve-out:** skip dataset / index / central bank / institutional acronyms ({{> acronym_carveout }} / `(Fed, 2022)`, and similar) and non-citation parentheticals (`equation (2.3)`, `Section 4 (1990–2020)`). If a candidate cite is neither tagged nor present in the paper's bib file (Read `paper/bib.bib` if present — fallback to `paper/references.bib` or `references/references.bib`; if no bib file exists yet, skip the paper-anchored exception and proceed to strip — then string-search the surname in the bib file's contents), replace the author-year phrase with `[CITE-STRIPPED]` and append a line `> Triager note: stripped unverified citation "{verbatim phrase}" — no [openalex:…] / [doi:…] tag, not paper-anchored.` immediately under the affected sub-header. The deepen directive's substance survives; the unverified cite does not propagate to theory-generator / empiricist.
...
5. **Polish bucketing (Stage 9 only).** Polish findings bucket as follows:
   ...
   - **Disposition pass-through (polish-prose rows).** Every `polish-prose` finding carries a `Disposition` field in its source report (`cut` | `compress` | `rewrite` | `add` | `rebalance`; items 1–9 are always cut/compress/rewrite, items 10–13 are `add`/`rebalance`). Copy that value verbatim into the `Disp.` column of the Apply/Investigate row so paper-writer reads it from the authoritative triage file without re-opening the source report. For non-polish-prose rows leave `Disp.` blank (their fixes are corrections by construction). A row with `Disp. = add` or `rebalance` is **additive**: it asks paper-writer to add a small amount of prose or rebalance sections, never to delete — and per rule 6 below it is not subject to the cut-vs-fix drop. Bucket additive findings on their own severity exactly like any other finding (an item-10 `major` with no verbatim quote of offending prose — because the defect is *absence* — routes to `Investigate` under the `major` rule above; that is correct, additive judgments get orchestrator review).
   - `informational` → `Investigate`. The polish-numerics "Methods reaching Stage 9 without canonical-package use" subsection (issue-#36 soft guard for post-pipeline-edit bypasses) emits findings tagged `informational` to flag named methods whose active receipt-declared analysis entrypoints appear hand-rolled despite the paper-prose claim of canonical-package use. Route these to `Investigate` with the standardized note: "method-checker did not re-fire after this edit; operator should assess whether a post-pipeline method change occurred and whether the prose attribution matches the actual implementation." The operator decides whether to re-fire `method-checker` on the active entrypoints or accept the finding as known. Do NOT `Apply` (the polish-numerics report did not author a concrete code fix) and do NOT `Drop` (the soft guard is the only catch for the issue-#36 post-pipeline-edit bypass and dropping it defeats the safety net).
   - **Dedup by anchor.** When two polish agents flag the same anchor (e.g., polish-institutions and polish-bibliography both flag {{TRIAGER_DEDUP_EXAMPLE}}), merge into one row tagged with both source agents and use the higher severity. When the deduped agents propose **contradictory** suggested fixes (e.g., polish-formula says "the equation is wrong, replace it" while polish-consistency says "the equation is right, fix the prose"), use this precedence to pick the fix to record in the row: `polish-formula` > `polish-numerics` > `polish-equilibria` > `polish-identification` > `polish-consistency` > `polish-institutions` > `polish-bibliography` > `polish-prose`. The intuition: ground truth is the math (formula > numerics), then the model structure / causal design (equilibria > identification), then cross-paper consistency, then the world outside the paper (institutions > bibliography), then editorial economy last (prose). Note the dropped fix in the row's Notes column ("polish-consistency proposed an alternative fix; superseded per precedence rule"). Both agents stay in the source-agent list, the loser's fix becomes a candidate for paper-writer to consider only if the winner's fix fails.

6. **Removal-vs-fix precedence (Stage 9 only).** This rule fires only on a polish-prose *cut* (`Disp. = cut`, or a `compress` whose suggested fix deletes an anchored instance). Polish-prose *additive* findings (`Disp. = add` or `rebalance` — items 10–13) are never dropped by this rule, because they add or relocate rather than delete the substrate another agent edits. When `polish-prose` proposes *cutting* prose at an anchor (or instance of a multiply-anchored finding) and *any other polish agent* flags the same anchor with a suggested fix that is *not a deletion* (a correction, rewrite, rephrasing, or addition — anything that edits the substrate rather than removing it), drop the polish-prose finding for that anchor with the justification "polish-prose cut superseded by [agent]'s non-deletion fix at same anchor — the prose is load-bearing for the [agent]'s finding." This rule extends rule 5: rule 5 settles same-anchor *fix-vs-fix* conflicts by recording the loser's fix as a fallback in Notes; rule 6 settles *cut-vs-fix* conflicts by dropping the cut entirely with no fallback, because deletion eliminates the substrate the fix would have edited. The rule fires per-anchor, not per-finding: a polish-prose finding with five anchors loses only those anchors that conflict; the others remain. If a polish-prose finding loses *all* its anchors this way, the entire finding moves to `Drop`. (Note: a finding from another agent that does not include a suggested fix at all does not trigger rule 6 — only an explicit non-deletion fix does. This is the rare case where another agent merely notes a concern without proposing an edit; in that case rule 6 does not fire and the polish-prose cut may apply.)
...
| # | Source agent(s) | Severity | Disp. | Anchor | Concern (one line) | Suggested fix | Notes (if downgraded from default) |
|---|------------------|----------|-------|--------|--------------------|---------------|-------------------------------------|
...
| 3 | polish-prose | critical | add | §1 introduction | Intro establishes neither {{PP_STAKES_TERM}} nor a literature anchor | Add a stakes sentence on why the magnitude matters; anchor against gennaioli2015neglected (in references.md) | (`Disp. = add` per rule 5 pass-through — additive, not a cut) |
| ... |

(`Disp.` is populated only for `polish-prose` rows — `cut`/`compress`/`rewrite`/`add`/`rebalance`; blank for every other agent. See rule 5 "Disposition pass-through.")

## Investigate (orchestrator decides; paper-writer drafts a candidate fix)

| # | Source agent(s) | Severity | Disp. | Anchor | Concern | Why this is Investigate, not Apply |
|---|------------------|----------|-------|--------|---------|------------------------------------|
| ... |

(`Disp.` populated for `polish-prose` rows only, same as the Apply table.)
...
## Summary
- Total findings across all polish reports: N
