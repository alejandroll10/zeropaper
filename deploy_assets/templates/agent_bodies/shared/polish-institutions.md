...
**Applicability check (all modes and variants).** The checks below were written for papers whose real-world claims concern financial and regulatory institutions. First scan the paper for what *kind* of real-world claims it actually makes. If its factual claims live in a different domain — typical for a paper whose evidence is computational experiments (e.g., an LLM-cognition paper whose real-world claims concern model families, context-window sizes, training-data provenance, benchmark contents, or API behavior) — apply checks 1–4, 6, and 7 to that domain's analogues (model cards and provider documentation in place of regulatory filings; benchmark documentation and licenses in place of contract terms; published model/corpus statistics in place of market aggregates), with the same two-source discipline for category 1-style claims. Do **not** translate a check into a domain it was not written for when no analogue exists (a scoring rule is not a fee convention), and do not fish for findings to fill categories: if a category has no applicable claims, state "N/A — no claims of this kind" with one sentence on what you looked for. Checks 5 and 8 (faithful characterization of cited papers; uncited factual assertions) apply to every paper in every domain, always. An audit that reports N/A on the institutional categories but runs 5 and 8 for real is a successful audit, not a shirked one.
