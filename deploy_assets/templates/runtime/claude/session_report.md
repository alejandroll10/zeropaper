...
AFTER THE SYNTHESIZER PRODUCES `report/referee_report.md`, LAUNCH THE REGISTERED `report-reviewer` AGENT TO REVIEW THE REPORT AGAINST `audits/*.md` AND `submission/`. FOR ROUND N, REQUIRE `process_log/report_self_review_r{N}.md` WITH `CLEAN` OR `FIX` AS ITS FIRST NON-EMPTY LINE. IT CHECKS THREE THINGS: (1) IS EVERY MAJOR CONCERN TRACEABLE TO AN AUDIT FILE OR EXPLICITLY MARKED AS A SYNTHESIZER NOTE? (2) DO THE STRENGTHS ACCURATELY REFLECT WHAT THE PAPER DELIVERS? (3) IS THE VERDICT CONSISTENT WITH THE WEIGHT OF CONCERNS RAISED? IF IT RETURNS `FIX`, GIVE THAT EXACT ARTIFACT TO A FRESH `report-synthesizer`, INCREMENT N, AND RE-REVIEW. ITERATE UNTIL `CLEAN`.

The registered role is pinned to the `sonnet` model. The review is on the *report*, not on the submission — we never edit the submission.
