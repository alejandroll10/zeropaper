...
if os.path.exists(state_path):
    ...
    emp_loops = {
        ...
        "unowned_failure":              {"round": 0, "cap": 4},
        "audit_fix":                    {"round": 0, "cap": 5},
        "stage3a_upstream_return":      {"round": 0, "cap": 2},
...
if os.path.exists(state_path):
    ...
    data.setdefault("loops", {})
    for _lid, _cap in (("spec_audit_revision", 3), ("coverage_certificate_producer", 3),
                       ("coverage_audit", 3), ("plan_review", 3), ("claim_discipline", 2)):
        data["loops"].setdefault(_lid, {"round": 0, "cap": _cap})
    with open(state_path, "w") as f:
        json.dump(data, f, indent=2)
        f.write("\n")
PYEOF
